...
- **Hatask App・Hataskey App**: 機能ごとのカードと外枠を統一し、説明の文末の句点を省きました。Hataskey App見出しの上の小さなHatask表記と、見出し直下の罫線を取り除いています
- **特集バナー**: PC・モバイルとも特集の高さを抑えました。切り替え・一時停止ボタンは同じ位置に残し、特集の内容だけがスライドします
- **ヘッダー内の検索**: 暁では、入力したあとに別の検索欄を開かず、元の入力欄の下へ結果を表示します。フォーカス枠を検索ボックスの角丸に合わせ、結果は上下に滑らかに開閉します。閉じた結果はキーボード操作の対象から外し、Escで結果だけを閉じます。従来テーマの検索画面は維持します
- **みんなのお花**: 暁の並び替えをN（New・新しい順）とO（Old・古い順）で表示します。読み上げとツールチップでは意味を確認できます。自分のお花は従来の並び替え名を使い、PCの非選択ボタンをアイコンのみ表示します。従来テーマの並び替え表示は維持します
...
- アプリ内の更新案内とREADMEをhata-12.5.3へ更新しました。更新案内は日本語・英語・簡体字中国語を揃え、hata-12.5.2とhata-12.5の案内も切り替えて読めます
