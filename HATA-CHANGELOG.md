...
設定画面を一から作り直した更新です。左に大分類、右にその中の項目をタブで並べる2枚組みへ改め、これまで別窓で開いていたHataskey独自の設定も右側にそのまま表示するようにしました。

### 表示・操作の改善

- **設定画面の再編**: 左ペインを大分類だけの縦の錠剤リストに、右ペインを同じ分類の項目を切り替えるタブに作り直しました。分類を選んだ時点でその分類の最初の項目へ移動します
- **分類の見直し**: 「データと連携」に集中していた項目をほどき、Hataskey UI・HataSNSCordUI・Hataskeyの道具・表示と外観・タイムラインと投稿・通知とサウンド・アカウントとログイン・ミュートとブロック・ドライブ・データと引越し・外部連携・Misskey UI・その他・CherryPick の14分類に組み直しました。プロフィールとアバターデコレーションはアカウントの分類へまとめ、「表示するタイムライン」はMisskey UIの分類へ移しました
- **Hataskey独自設定の埋め込み**: 地震・津波情報、Hatadyの表示、設定の引き継ぎ、Hataskey UIの編集、UIの選択、Hatask、マスコットの7つを、別窓ではなく設定の右ペインへそのまま表示するようにしました
- **左上の絞り込み**: 「すべて」「よく使う項目」「この端末のみ」を、タブと同じ錠剤ケースの形に揃え、それぞれALL・星・スマートフォンのアイコンで表しました。折りたたみのボタンは独立した丸ボタンとして分けています
- **タブレット・折りたたみ端末**: PCと同じ2枚組みの表示に揃えました
- **スマートフォン**: 分類を選ぶとそのまま項目へ移動するようにし、進む・戻るに滑らかな遷移を付けました。詳細画面では検索窓を上部の虫眼鏡ボタンへ畳み、一覧へ戻ると検索窓の形に開き直します
- **無操作時の案内**: スマートフォンの詳細画面で15秒間なにも設定を変えていないとき、上部の見出しを一時的に「こちらから設定を検索できます ＞」へ差し替え、8秒後に元の見出しへ戻すようにしました
- **スクロールバーを表示しない**: Hataskey UIのサイドメニューに合わせ、設定の左右どちらのペインでもスクロールバーを表示しないようにしました
- **Hataskey UIのプレビュー**: UIの選択画面と同じ見本になるよう作り直し、透明度・すりガラスなどの変更がプレビューへ反映されるようにしました
- 「今回の更新内容」の2番目に、この設定の作り直しを見本付きで追加しました
- **HataSNSCordUIのアイコン刷新**: HataSNSCordUIが所有する画面のアイコンをpqoqubbw/icons由来のアニメーションアイコンへ入れ替え、従来の独自アイコンアニメーションを撤去しました。Hataskey UIは変更していません
- **投稿ツールの縦スワイプ選択**: 星メニューを投稿欄の通常フローから高優先ポップアップ層へ移し、起点ボタンの真上へ表示する縦スワイプ式に変更しました。PCでは5項目、スマートフォンでは4項目を一度に表示し、各項目はタップまたはクリックで実行します

### 修正

- 「今回の更新内容」の「iPhone・スマホの操作を安定化」の見本が、左上へ潰れて表示される問題を修正しました。見本の土台となる指定が抜けていました
- Hataskey独自機能の分類で、元が別窓だった項目を選んだあとに別のタブへ移ってもタブの選択表示が残る問題を修正しました
- 左ペインの項目名が「...」で途切れる問題を修正しました
- 「ドライブの整理」のタブにアイコンが表示されない問題を修正しました
- 絞り込みと「よく使う項目」の見出し・項目名が左に寄る問題を修正しました
- スマートフォンの見出しが中央に揃わない問題を修正しました
- 左上の戻るボタンで設定から抜けられず、子の分類と設定の入口を行き来し続ける問題を修正しました
- Misskey UIの分類で、非推奨のバッジの下に余白が残る問題を修正しました
- デッキUIで旧設定を開くと上部のバナーに戻るボタンが隠れる問題と、旧設定へ移ったあとマウスホイールでのスクロールがまったくきかなくなる問題を修正しました
- デッキUIで最大化と通常表示を切り替えると右ペインが白いままになり、左から何を選んでも戻らない問題を修正しました
- タブレット表示で左上のプロフィールへ移るボタンが無い問題を修正しました
- HataSNSCordUIの星メニューを開くと投稿ドックが膨らみ、タイムラインのノートが途切れて見える問題を修正しました
- HataSNSCordUIの星メニューが投稿欄から離れた位置へ表示され、メニュー外を操作しても閉じない問題を修正しました。外側操作とEscで閉じ、起点ボタンへフォーカスを戻します

### 素材とライセンス

- pqoqubbw/icons由来のLucide Animatedアイコンと、その実行依存であるMotion for Vueのライセンス文書を同梱しました

### 連合への影響

- 今回のHataSNSCordUI変更はフロントエンドの表示と操作だけを変更し、ActivityPubによる連合の送受信には影響しません

## hata-12.3

Hatadyにゲーム・映画の記録を追加し、投稿分析HATAlyzeを内製化し、Hataskへコルクボードの新テーマ「ハタキュ」を加えた大型更新です。あわせてHataskeyオリジナルアイコンへのブランディング刷新、横開き折りたたみ端末への対応、画面各所のアニメーション再設計を行いました。

### 新規機能

- **Hatadyのゲーム・映画の記録**: 本に加えて、ゲームのプレイと映画の鑑賞を記録できるようにしました。「コレクション」から作品を登録し、プレイ・鑑賞ごとの記録を残せます。おすすめ度、記録の種類、タイトル・制作者の検索と詳細な絞り込みに対応します
- **ゲーム記録の詳細**: 協力プレイの記録、武器ごとの成績表、入力候補付きのタグ入力を追加しました
- **HATAlyze（投稿分析）の内製化**: 端末内で完結する投稿分析を追加しました。感情を喜び・楽しさ・親愛・感謝・怒り・悲しみ・不安・疲れの8軸へ振り分け、用語集を501語へ拡充し、曜日・時間帯・話題・投稿習慣・言葉づかい・反応との関係を集計して表示します。保存するのは集計値だけで、投稿本文・投稿ID・投稿日時は履歴に残しません。よく使った言葉は端末内の表示のみで、履歴には保存しません。保存できる履歴は最新5件までとし、超過分は古いものから削除します
- **Hataskの新テーマ「ハタキュ」**: コルクボードに紙をピンで留めた見立ての新テーマを追加しました。初回のテーマ選択にも並び、Hatask設定から切り替えられます。紙は常時ゆれ、ときどき突風で大きく動きます。風はこのテーマ限定の設定で止められます
- **Hataskeyオリジナルアイコンのブランディング**: 設定メニュー、各設定ページのバナー、エラー表示、ダイアログ、ログイン画面、新規登録の流れ、オフライン画面のアイコンを、Hataskeyオリジナルの立ち絵へ差し替えました。設定からいつでも従来のアイコンへ戻せます。管理画面で設定したブランディング画像は、この設定に関わらず従来どおり最優先されます
- **横開き折りたたみ端末への対応（ベータ）**: 折り畳み端末のメインディスプレイで、表示はモバイルのまま右側にウィジェット（Hataskey UI）・サブペイン（HataSNSCordUI）を常時出せるようにしました。判定は幅と入力方式で行い、機種名では判定しません。設定は自動・オン・オフの三値で、折りたたみ端末と通常のスマートフォンで同じアカウントを使っても片方が壊れないよう、端末ごとに保存しほかの端末へ同期しません。判定に使う幅の値は実機で測る前の暫定値です
- **外部アカウント連携の接続先に㐂五亭を追加**: 㐂五亭 (ddoskey.com) につなげるようにしました。旗鯖以外の外部サーバーのため、つないだ先の規約が適用されます
- **HataFeedの絵文字申請の保留**: 管理者が申請を後回しにするとき、入力し直しにならないよう入力値ごと保存できるようにしました。保留中も承認・却下でき、一覧では専用の絞り込みと色で見分けられます。申請者とスタッフの双方へ通知します
- **入力内容の下書き保存**: 絵文字申請・不具合報告・要望・Hatadyの記録と本の登録で、書きかけの内容を保存し、続けて申請できるようにしました
- **Hatadyの通知**: フォロー、コメント、リアクション、連続記録の節目をプッシュ通知で受け取れるようにしました。あわせて自分の記録に付いたコメントを削除できるようにしました（返信は残ります）
- **Hatadyの書き出しの拡張**: 書き出しが映画・ゲームの記録にも対応しました。読みやすい文字形式を選べ、期間と種別で絞り込み、複数の種別をまとめて書き出せます
- **ミュートした人のリアクションを隠す機能を正式化**: ベータ機能から正式な機能へ移行しました
- **ロールの追加**: Hatadyで記録できるゲーム作品数の上限をロールごとに設定できるようにしました（既定100、最大1000）。対戦・プレイ記録の件数ではなく、異なる作品数の上限です
- **カスタムフォントのアップロード**: Hataskey独自設定のフォント画面からTrueType（`.ttf`）とOpenType（`.otf`）を直接アップロードし、UIへ適用できるようにしました。フォントは正しいMIMEタイプで配信します。アップロード可能なファイル種別は従来の既定値を使用しているサーバーだけを自動更新し、サーバー管理者が独自に設定した許可リストは変更しません

### 表示・操作の改善

- 投稿フォームの送信状態と操作アニメーションを再設計しました。待機中・送信中・送信完了・送信失敗が見て分かるようになります
- プロフィールカード（利用者のポップアップ）を再設計しました
- Hatadyの統計表示に、値が順に立ち上がる段階アニメーションを追加しました
- Hatadyのマイログに表示種別の切り替えを追加し、コレクションに入場演出を付けました
- HataSNSCordUIの投稿とナビゲーションを改善し、専用の公開範囲ピッカーを追加しました
- Hataskey UIのナビゲーションとデッキ操作を改善しました
- HataSNSCordUIのマスコットとUIアイコンの素材を追加しました
- 「今回の更新内容」を今回の内容へ入れ替え、項目ごとのプレビューを作り直しました。プレビューには文言を持たせていません（訳の無い言語で空欄になるため）。開いたときにカードが左上から右下へ順に現れます。動きを減らす設定では、これらの動きはすべて止まります
- HataFeedの通知本文に含まれる絵文字を、絵文字として表示するようにしました
- Hataskの独自画面での絵文字表示を統一しました
- HataFeedの通知一覧は、ベルから開いた場合と標準の通知一覧から見た場合のどちらでも、表示した時点でバッジが消えるようにしました
- コメント・画面文言・ドキュメントに残っていた「HatasabaUI」「Hatasaba UI」の表記を「Hataskey UI」へ揃えました。挙動の変更はありません

### 修正

- 言語を設定画面で保存していない場合に、画面の文言は日本語なのに日付・曜日・お花の名前・Hatask Eyeの文言だけ英語で表示される問題を修正しました。起動時に読み込まれた言語へ揃えます
- Hataskの気持ちリマインドが同じ時刻に何通も届く問題を修正しました。予定の通知とタイマーの入れ物を共有していたため、リマインドを張り直すたびに古いタイマーが残り、さらに予定を追加するとリマインドのタイマーが消えていました
- スマートフォンでリアクションを長押ししたときの操作を改善しました
- カードメーカーで、カード画像のアイコン装飾の位置がずれる問題を修正しました
- カードメーカーで、タッチ操作がページのスクロールと競合する問題を修正しました
- HataSideStudioの配置判定と、移動時の案内表示を修正しました
- HataSNSCordUIで、宴の枠・公開範囲アイコン・使用量サークルの表示を修正しました
- HataSNSCordUIの右ペインを操作している間、入力欄以外へ文字入力カーソルが表示される問題を修正しました。要素ごとに塞ぐのではなくUI全体で消し、入力欄だけ元に戻します
- Hataskey UIのスマートフォン表示で、ホームボタンを押すとタイムラインのタブが強制的にホームへ切り替わる問題を修正しました
- 一部の独自機能で、選択を空にして保存できない問題を修正しました
- 宴で、ホームに公開した投稿を成功・阻止に数えないよう修正しました

### 素材とライセンス

- 追加したイラスト・アイコン素材の権利とライセンスを明記しました

### 連合への影響

- フォントファイルの対応はローカルサーバーへのファイルアップロード判定だけを変更し、ActivityPubによる連合の送受信には影響しません

## hata-12.1.3

iPhoneでのノート操作、カードメーカー、言語の初期設定を改善する修正版です。

### カードメーカーの改善

- 「旗鯖カードメーカー」を「カードメーカー」へ改名し、不要になった「Hataskツール」とログイン中プロフィールの案内表示を削除しました
- プロフィールの二次元コードを、サーバーアイコン、丸いドット、角丸の位置検出パターンを使った本体の二次元コード画面に近いデザインへ変更しました
- 対応するiPhoneでは、利用者が許可した場合に限り、端末の動きと画面の向きに合わせてカードを傾けられるようにしました。追加の端末動作ライブラリや外部通信は使用しません
- iPhoneの端末傾き操作で光沢だけが移動し、カード本体が傾かない場合がある問題を修正しました。Safariでもカード面へ直接3D変形を適用します
- アクセント色などのボタンを操作した際、入力欄のない画面へ文字入力カーソルが残って点滅する問題を修正しました。キーボード操作用のフォーカス表示は維持します

### iPhoneのノート操作の修正

- iPhoneで返信、リノート、リアクション、ノートメニューなどを押しても反応しなくなることがある問題を修正しました。タッチ開始からクリックまでの間にツールチップを追加せず、ノート本来の操作を優先します
- iPhone PWAでリアクションボタンは反応してもリアクション選択メニューが開かず、キャッシュ消去で一時的に直る場合がある問題を修正しました。Misskey 2026.5.0の本流修正に合わせ、リアクション・絵文字選択画面をタップ処理内で同期的に開きます
- HataSNSCordUIではノート内ボタンより先に外側の投稿欄がクリックを捕捉しないようにし、画面端スワイプが中断された場合も操作状態を確実に解除します

### 言語設定の修正

- 表示言語が未設定の既存ローカル利用者を、日本語（ja-JP）へ補正するマイグレーションを追加しました。既に選択されている言語とリモート利用者のプロフィールは変更しません
- 新しく登録するローカル利用者の既定言語も日本語とし、言語設定が空欄のまま作成されないようにしました

## hata-12.1.2

Hataskのカードメーカーとお花の成長、HataSideStudio、Hataskey UIデッキ、HataSNSCordUI、情報ページを改善する機能更新です。

### 新規機能・設定

- **旗鯖カードメーカー**: ログイン中のプロフィールを使い、Hatask内でデジタル通行証カードを作れるようにしました。追加のログインやサーバー選択は不要で、通常・ゴールドの外観、アクセント色、透明度を調整し、端末内でPNG画像として保存できます
- **HataSideStudioのノートボタン設定**: サイドメニュー下部のノートボタンを鉛筆または猫の肉球アイコンから選び、背景色・文字色・グラデーションを設定できるようにしました。保存した設定は通常表示とスマートフォンのサイドメニューへ反映されます

### Hatask・Hataskey UIの修正

- Hataskのお花をページの表示状態に依存せず実経過時間で育て、画面を閉じている間やバックグラウンドの時間も次回起動・復帰時に反映するようにしました。花のレジストリをまだ持たない利用者でも初回の花が保存され、育ち始めます
- Hataskey UIデッキでツールバーの時計を表示したとき、秒表示の更新に合わせて通知カラムやほかのカラムが再描画されて点滅する問題を修正しました。時計だけを独立して更新し、カラムへ渡す設定値も安定した参照で保持します

### HataSNSCordUI・画面表示の修正

- HataSNSCordUIで返信を選んだとき、返信先アカウントのアバターが横長になる問題を修正しました
- HataSNSCordUIとUI選択画面でポップアップを開いた際、背景左上へキャレットが残り、背景のぼかしが点滅する問題を修正しました
- HataSNSCordUIの設定で、ダークテーマ中にUIサイズの選択ボタンへポインターを合わせると配色が反転して読みにくくなる問題を修正しました
- 「Hataskeyについて」とローカル・連合先のサーバー情報ページを、日本語・英語・中国語（簡体字）の共通言語設定へ対応させました

## hata-12.1.1

HataFeed通知の既読状態を正しく反映するための修正版です。

### 通知の修正

- HataFeed通知をHataFeedまたはHataskから開いたとき、その通知だけを既読にし、未読件数へ即時反映するよう修正しました。「すべて既読」の操作は従来どおり利用できます

## hata-12.1

HataSNSCordUIを追加し、画面を切り替えずにHataskeyの主要機能を並行して扱えるようにした機能更新です。

### 新規機能

- **HataSNSCordUI**: 左にタイムライン・リスト・アンテナ・チャンネルと機能メニュー、中央に会話型タイムラインと投稿欄、右に複数タブのサブペインを置いた新しいUIを追加しました。UI選択画面から切り替えられます
- **UI内で完結する画面遷移**: 設定、コントロールパネル、旗鯖独自ツール、ゲーム、プロフィールなどを開いてもHataskey UIへ戻らず、左右のペインを維持したまま中央ペインで表示します
- **右サブペインの作業タブ**: 投稿詳細、プロフィール、検索、通知、お知らせ、ドライブ、チャット、各タイムライン、リスト、アンテナ、チャンネル、ウィジェットを追加・削除・並び替えできます。Hataskey UIのウィジェット構成も取り込めます
- **会話型タイムライン**: 自分とほかの利用者の投稿を左右へ分け、既存のリアクション、返信、引用、投稿メニュー、チャンネル表示を保ったまま会話風に表示します。新着投稿は下へ滑らかに追加されます
- **通知・アクション表示**: 通知や操作結果をタイムライン内へ順番に表示し、強調後は詳細を折りたためます。地震・津波情報は遅延させず即時表示します。このUIの利用中だけ通常のトースト通知を止め、ほかのUIへ戻ると復帰します
- **初回チュートリアルと実績**: 初回起動時に操作方法とUI専用レートリミットを最後まで案内し、完了時に専用実績を解除します
- **旗鯖独自機能の英語・中国語（簡体字）対応**: Hatady、Hatask、HataFeed、HataSNSCordUI、HataSideStudio、Hataskey UI、旗鯖独自設定、外部アカウント連携・通知、登録申請、マスコット、お絵描きツール、独自ゲームなどの表示を、本体の共通言語設定へ統合しました。Hatadyの独自言語選択は廃止し、保存済みの記録や設定を変えずにHataskey本体の表示言語へ従います。利用者が入力した投稿・記録・名前は翻訳しません。花常と地震・津波情報は対象外です

### 管理・設定

- **ロール制御**: HataSNSCordUIは既定で利用可能です。ベースロールと個別ロールから利用可否、右サブペインの最大タブ数（1〜5）、1時間あたりのUI専用操作上限（既定500、最大1000）を設定できます
- **端末ローカル設定**: 左メニューの表示・順序・ピン留め、右ペイン、ウィジェット、配色、レートリミット数値表示を端末内へ保存し、旗鯖独自設定の書き出し・読み込み対象に追加しました
- **UI専用の共通操作上限**: HataSNSCordUIから送る認証済みAPI操作だけを数える上限と残量表示を追加しました。通常UI、外部アプリ、ActivityPubの連合処理には適用しません

### 表示・操作の改善

- ライト、ダーク、現在のテーマカラーを切り替えられ、文字・アイコン・投稿・ポップアップを含めて読みやすい配色へ揃えました
- 左メニューは縮小表示、端末ごとの並び替え、ピン留め、「もっと！」との出し入れ、外部タイムラインに対応しました
- 投稿欄は公開範囲、ローカル限定、添付、CW、アンケート、イベント、返信、引用、メンション、チャンネル投稿、投稿前カウントダウンに対応しました
- 投稿欄の公開範囲別の色枠は、本文入力へフォーカスしている間だけ表示します。投稿待機中は進捗リングを優先し、スマートフォンでは「今すぐ投稿」をプレビューより上へ配置します
- メインタイムラインのリアルタイム更新を、ノート追加だけでなくリアクションの追加・解除、アンケート投票、本文・CW編集、宴の状態更新、ノート削除、アンテナからの除外まで既存の更新経路へ接続しました。外部アカウントのタイムラインも対応するノート更新を購読します
- タイムラインを操作したときは散らばった通知をひとまとめにし、通知本文を読み返せる状態を保ったまま画面占有を抑えます
- スマートフォンでは左端から左メニュー、右端から右サブペインを開くよう分離し、両方が同時に重ならないようにしました。右ペインの追加メニューも一列ずつ選ぶ表示に変えています
- HataSideStudioのスマートフォン編集画面を整理し、簡易タイムラインの挿入位置をタッチ座標から判定できるようにしました。削除と高度な並び替えは左側へ揃え、機能を省かず上部操作を横スクロールできます
- 暗色表示でUIサイズ選択の選択中・未選択が反転しないよう、文字・枠・背景を専用配色へ揃えました
- レートリミットの円表示は既定で数値を隠し、設定で残量率を表示できます
- レートリミットの詳細は「活用できます」「利用する準備ができています」と、残量状態に合う案内へ改めました
- 旗鯖機能解説と今回の更新内容にも言語対応の説明と設定への導線を追加し、日本語・英語・中国語（簡体字）の各表示で読めるようにしました

### 通知の改善

- 通知ウィジェットで「プライベートチャンネルへ追加」「プライベートチャンネルから除外」のどちらかを非表示にすると、通知一覧そのものの取得に失敗する問題を修正しました
- 通知フィルタは保存時点の通知種別を記録し、今後追加される新しい通知種別を利用者の確認なしに自動で有効にしないようにしました。既に通知フィルタを設定している利用者には、除外設定を通過する一度限りの案内を通知欄へ表示します
- Botフラグ付きアカウントからの通知を、通常の通知メニュー、通知ウィジェット、Hataskey UIデッキ、HataSNSCordUIのサブペインで非表示にできるようにしました
- HataSNSCordUIではBotと人間からの通知を別々にまとめ、交互に届いた場合も混在させず「Botからの通知」として判別できるようにしました
- HataFeedとプライベートチャンネルの通知は、英語・中国語（簡体字）表示でもイシュー名、状態、絵文字名、却下理由、チャンネル名などを失わず、画面内とOS標準通知の双方へ表示するようにしました。未知の通知文は消さず原文を表示します
- HataSNSCordUIの通知履歴は、表示済みの翻訳文ではなく通知の種類と必要な情報を端末内へ保存し、言語変更後も履歴を失わず現在の表示言語で読み直せるようにしました。保存するノート情報には件数と長さの上限を設け、端末内キャッシュが過度に増えないようにしています

## hata-12.0

新規機能を追加し、独自機能を改善した上でベースを Misskey **2026.7.0** 相当へ更新する大型アップグレードです。

### 新規機能

- **Hatady (ハタディ)**: 学習と読書の記録、目標、連続記録、本ごとのしおりと内容メモ、公開範囲、横断検索、書き出しを備えた新しいツールを追加しました
- **花常 (はなつね)**: 花を並べて消すパズルに、十二か月の物語、街の様子、花の名鑑、季節イベントを組み合わせた新作ゲームを追加しました
- **Hatask の全面リデザイン**: ホーム画面を「季」「花信」「刷」の3テーマから選べるようにし、HataFeedや地震・津波情報をまとめて確認できる表示を追加しました
- **HataSideStudio**: 実際のサイドメニューを見ながら、ボタン・グループ・全ウィジェットの配置、形、配色、サイズ、表示内容を端末ごとに編集できるスタジオツールを追加しました。拡大時と縮小時の個別構成、複数プロファイル、ドラッグと簡易タイムライン、元に戻す・やり直し、モバイル表示、設定の書き出し・読み込みに対応し、Hataskとサーバーメニューからも起動できます。Hataskey UIデッキ上でも作成・確認・チュートリアルの小窓が手前に表示されます
- **Hataskey UI 2**: ノート、カラム、上部・下部のメニューをすりガラス調で統一する新しい表示を追加しました。透過の強さなどは独立した設定画面で確かめながら調整できます
- **Hataskey UIデッキの機能追加**: クリップ・お気に入りのカラム、カラムごとの再読み込み、チャンネル投稿ボタン、ウィジェット編集への導線を追加しました
- **Bot投稿の非表示**: Botの投稿をタイムラインから隠し、必要なBotだけを例外として表示できる設定を追加しました
- **Hataskのお花ウィジェット**: 育成中の花を円形の進み具合で示し、咲いた花の一覧と総数を通常表示とHataskey UIデッキの両方で確認できるようにしました
- **プロフィールの特殊バッジ**: 自サーバーの利用者プロフィールに、宴の成功回数・宴の阻止回数・育てた花の数を表示できるようにしました。本人はプロフィール設定から項目ごとに表示を切り替えられます
- **今回の更新内容**: 更新後に主な変更を案内する画面を追加し、「もっと！」→「ヘルプ」→「直近の更新内容」から後で読み直せるようにしました
- **管理者向けチャンネル一覧**: 管理者とモデレーターが公開状態にかかわらずチャンネルを確認できる一覧を追加しました

### ベータ機能

- **C/C++ プレイグラウンド**: ブラウザ内だけでC/C++を書いて実行できる遊び場を追加しました。コードはサーバーへ送信されません
- **投稿前カウントダウン**: 通常投稿・返信・引用で、送信前に3秒・5秒・10秒の猶予を作れるようにしました。待機中は取り消しまたは今すぐ投稿を選べ、設定と待機は端末内で完結します

### Hataskey UI・画面表示の改善

- **左右スワイプの停止設定**: Hataskey UIのタブを左右スワイプで切り替えないよう、端末ごとに設定できるようにしました。初期状態は従来どおり有効です
- **チャンネルとプロフィールのタブを整理**: 選択中の項目が分かりやすい丸みのある表示へ統一しました
- **Hataskey UI 2の表示を安定化**: チャンネル色の縦線、日付の区切り、Bot非表示後の空白、従来デッキとの投稿ボタン重複などを修正しました
- **設定画面を見比べやすく**: Hataskey UIとサイドバーの編集画面を、背後の表示を操作しながら調整できる独立ウィンドウへ変更しました
- **UI選択画面の見切れを修正**: 画面より内容が高いときに下までスクロールできるようにし、スマートフォンの安全領域にも対応しました
- **「みつける」と「もっと！」の戻り道を修正**: Hataskey UIで戻るボタンが出ない問題と、マスコットがサイドバーにも「もっと！」にも現れない問題を修正しました
- **マスコットの最小化位置を安定化**: 最小化したマスコットが画面外へ移動しないよう、右下へ収まる表示に統一しました
- **絵文字表示の切り替えを修正**: 絵文字の表示方法を変更したとき、再読み込みしなくても表示へ反映されるようにしました
- **サブ管理者の表示を整理**: チャンネルの編集画面で、サブ管理者と権限の状態を確認しやすくしました
- **初回アクセスを安定化**: 初めて開いたときに画面上部だけエラーになる問題を抑え、登録方法を選ぶ画面の背景ぼかし点滅を修正し、Hataskeyロゴを見やすくしました

### 通知・HataFeedの改善

- **HataFeedを全面リデザイン**: イシュー一覧を検索・絞り込み・ロードマップ・通知がまとまった画面へ刷新し、詳細画面も会話の流れと状態・担当・賛同を同時に確認しやすい構成へ変更しました。スマートフォンでは情報を縦に整理して表示します
- **HataFeedの書き出しを改善**: イシュー番号・作成日・受付状態・ステータス・カテゴリで範囲を選び、本文・会話・コード・解決メモ・投稿者名・集計のうち必要な内容だけをJSONへ書き出せるようにしました。二重実行と保存開始直後のデータ破棄も防ぎます
- **絵文字申請を見つけやすく改善**: HataFeedのトップに「絵文字申請」と「新規イシュー」を左右に並べ、スマートフォンでも申請欄を表示するようにしました。スタッフは未処理の申請を、左右ボタンで切り替えながら1件ずつ連続確認できます
- **HataFeed通知の表示を改善**: 通知を開いたとき、背後のHataFeedまでぼかされないようにしました
- **HataFeed通知をまとめて表示**: 同じHataFeedに関する通知をひとまとまりにし、複数のHataFeedから通知がある場合も「複数のHataFeed通知があります」とまとめて確認できるようにしました
- **OS通知に旗鯖独自通知を表示**: Hatask、HataFeed、地震・津波、プライベートチャンネルの通知へ見出し・本文・移動先を付け、版番号だけが出る問題を修正しました
- **更新時の案内を順番に表示**: 更新内容、ログイン日数などの画面が重なって消えないよう、1つずつ順番に表示するようにしました

### Hatask・Hatady・お絵描きの改善

- **Hataskの廃止済み並び替えを撤去**: 現在は機能しないホーム項目の並び替え導線と設定を削除しました
- **Hataskのお花の成長を修正**: 実際に経過した時間を基準に育つようにし、ウィジェットから一覧と育成状況へ移動できるようにしました
- **Hatadyのスマートフォン表示を改善**: 「学習の記録」と「過去20週」のグラフを限られた幅で見やすくし、左右へのはみ出しも防ぎました
- **Hatadyに戻るボタンを追加**: 通常表示のパソコンとスマートフォンの左上から直前の画面へ戻れるようにしました。Hataskey UIデッキではデッキ側の操作と重複しないよう非表示にします
- **お絵描きツールを改善**: 投げ縄ツールの選択状態を見やすくし、プレビュー表示中でも確定・キャンセルを操作できる配置へ変更しました

### プロフィール・設定・外部連携の改善

- **プライベートチャンネルの招待を承認制に**: 管理者がメンバーを追加してもすぐには参加せず、招待された本人が通知から「参加する」を選んだときだけ参加するようにしました。チャンネルの管理画面では、招待中・参加中・招待拒否を確認できます。作成権限がない場合は、切替の下に管理者の許可が必要なことを表示します
- **プロフィールの見た目を調整**: ヘッダー画像と本文の境界を自然につなぎ、旗鯖バッジを中央揃えで表示するようにしました。これらのバッジは他サーバーの利用者には表示しません
- **利用データの書き出しを見つけやすく**: 旗鯖独自設定の入出力画面から、設定ファイルとは別にHatadyの学習記録とHataFeedのイシューを、それぞれの専用画面で書き出せるようにしました
- **ミュートした利用者のリアクションを非表示に**: ミュートした人が付けたリアクションをノートから隠せる設定を正式機能として追加しました
- **外部アカウント連携の接続先を整理**: 旗池3丁目とシュリンピアを候補から削除し、利用履歴がある場合はログイン情報と関連キャッシュを削除します。「les-requin」の表示名は「さめすきーとチョリソリング」へ変更しました
- **Botの例外選択を安全化**: Bot以外を例外へ追加できないようにし、以前保存された通常アカウントや削除済みアカウントも自動的に除外します
- **旗鯖機能解説を全面整理**: Hataskey UIとHataFeedの重複説明をまとめ、専門用語を利用者向けの表現へ改め、プライベートチャンネルと投稿範囲別の色分けを追記しました

### 花常 (はなつね) の改善

- **物語とパズルの流れを整理**: 物語を読み終えた後は未クリアの先の物語へ進まず、次のステージへ移る流れにしました。読み返しから戻ったときに物語が始まる問題も修正しました
- **ステージの区切りを明確化**: パズル開始時に目標を表示し、クリア後は結果を画面中央で案内してホームへ戻るようにしました
- **イベントを段階的に開放**: 最初から全ステージを見せず、クリアに応じて次のステージが現れるようにしました。開催期間などは管理画面から設定できます
- **パソコンとスマートフォンの表示を改善**: ホームの2列表示、物語の立ち絵と本文、街の様子、花の名鑑、イベント画面の見切れ・重なり・余白を修正しました
- **花常を終了する導線を追加**: メインメニュー左上からゲーム一覧へ戻れるようにしました
- **街の様子と物語を拡充**: 投稿、背景、人物の表情差分、物語本文を追加し、場面や時刻の変化が伝わるようにしました
- **音とクレジットを整備**: 環境音を利用条件を確認した音源へ差し替え、設定画面から効果音・環境音の出典を確認できるようにしました

### セキュリティ修正

- **HataFeedの書き出しで非公開カテゴリを保護**: プロジェクト作成者が書き出す場合も、通常の一覧・詳細と同じく「セキュリティ対応」カテゴリを存在ごと含めないようにしました。管理者・モデレーターの書き出しは従来どおり対象にできます
...
- **HataFeed の非公開化後も過去の参加者へ題名付き通知が届く問題を修正**: 一般公開イシューを後からセキュリティ対応カテゴリへ変更した場合、本文は見えなくなる一方、作成者や過去のコメント参加者には状態変更・返信・リアクション等の通知が届き、題名と存在が分かる状態でした。`feedbackId` を持つ全通知を本文と同じ `canViewIssue()` 判定へ通し、閲覧できない利用者にはHataFeed内通知・標準ベル通知のどちらも作らないようにしました
- **管理APIの外部アプリ境界を強化**: 機密設定の取得・更新、パスワード再発行、二要素認証解除、Hatady管理一覧・削除、花常イベント索引の取得・更新・統計の9経路に `secure: true` を追加し、管理者やモデレーター本人が許可した外部アプリ用トークンからも呼べないようにしました
- **Hatady APIへロール連動のレート制限を追加**: 全43経路を、軽量な読取・集計等の重い読取・書込・削除・管理削除の5段階に分けて基準値を設定しました。実際の上限は利用者のロールにあるレート制限倍率から動的に決まるため、通常操作を妨げずに過剰アクセスを抑えます
...
- 単体テスト 17 件を追加 (陽性対照で検出器の生存を確認済み)

## hata-11.8

Hataskey UI のグラスモーフィズム基調刷新版「**Hataskey UI 2**」をベータ導入する UI 大型リリースです。ノート面のガラス調・カラム全体の透過統一・独立ウィンドウ化した設定パネル (ライブプレビュー付き) など、視覚と操作の両面を刷新しました。あわせて Hataskey UI デッキ (Hataskey UI 2 デッキ) にクリップ/お気に入りタブ・カラム個別リロード・チャンネル投稿ボタンを追加、チャンネル/プロフィール UI を Hataskey UI 統一のピル型に刷新、管理者向けの全チャンネル一覧タブ、Bot をタイムラインに表示しない設定 (許可アカウント指定可)、Twemoji アセット復活とマスコット最小化位置の恒久化などを行いました。連合 (ActivityPub) への影響はすべての変更でゼロを事前検証済みです (UI/クライアント側と 1 個の管理系エンドポイントのみ、AP 配送経路・ノート生成ロジック・federation queue には一切変更なし)。

### 主要機能: Hataskey UI 2 (グラスモーフィズム刷新) ベータ

Hataskey UI に「Hataskey UI 2」というグラスモーフィズム基調のバリアントを追加しました。ノートカードをテーマ accent 由来のガラス面 + `backdrop-filter` で描画し、上部/下部ナビバーも同じガラス面で統一します。旗鯖設定 → Hataskey UI 2 タブ内の「設定を開く」ボタンから **独立ウィンドウ** を開き、ON/OFF・透過率スライダー (0-100%)・吹き出しモード時の追加ガラス・プロフィールバナー無し時の背景などを個別調整できます。
...
- **全ノート含有カラム/タブに透過率が反映**: Hataskey UI 2 有効時、ホーム/ローカル/グローバル/チャンネル/リスト/アンテナ/メンション/ダイレクト/外部 (OHTL/OLTL)/通知/クリップ/お気に入り/トレンドのすべてで、ノートカードが `--htk-glass-card-opacity` に連動して透過します。Hataskey UI 2 デッキ内の全カラムでも同じ挙動を保証
...
### Hataskey UI デッキ (Hataskey UI 2 デッキ)
...
- **チャンネル投稿ボタン再設計**: Hataskey UI 2 デッキ限定のフローティング投稿ボタンを新設。従来デッキUIには表示しません (`showChannelPostFixedButton` を `isHatasabaDeck.value` に限定して二重表示を防止)
...
- **チャンネル画面**: 検索/最新/ピン留め/カテゴリ 等のタブや、投稿一覧/ピン留めノート/フィード、ノート/リプライ/クリップ 等の切替を Hataskey UI 統一のピル型 (`MkTab` 拡張) に刷新しました。`MkTab.vue` にピル型バリアントを追加し、色/形/spacing を統一
...
- **Hataskey UI 2 の colorBar (チャンネル色縦バー) がリアクションを貫通する問題**: `mask-image: linear-gradient(to bottom, black 0%, black 55%, transparent 90%)` によるフェードアウトで修正 (DOM 構造は変えずに視覚的に消す)。あわせてアバターに重ならないよう Hataskey UI 2 モードでは `left: -10px` シフト + `border-radius: 20px` でノート外形と揃える
- **従来デッキUIで新規チャンネル投稿ボタンが二重表示される問題**: `showChannelPostFixedButton` を Hataskey UI 2 デッキ限定に変更
...
- **Hataskey UI 2 の日付セパレータ前後で白背景が残る問題**: `data-hatasaba-spacer="on"` ラッパの `> div > div` に対して `background: transparent` を強制し、MkNote root panel bg 残渣を除去
...
- **旗鯖設定タブ再編**: Hataskey UI 2 タブは「設定を開く」ボタン 1 個に集約 (独立ウィンドウ経由でアクセス)。旗鯖全体タブに Bot 非表示設定を追加。並び替え設定の書き出しモーダルも `MkWindow` 化
...
- **アンテナ管理画面の細部 UI 改善**: 旗鯖独自 Hataskey UI とのレイアウト衝突回避のためスキップ
...
- **Hataskey UI サイドバーから「メッセージ」「リロード」を非表示・並び替えできるように**: 旧実装は `ui/simple.vue` で chat (メッセージ) を動的注入、reload (リロード) をテンプレート内ハードコードボタンとして表示していたため、設定 UI のサイドバー編集一覧に表示されず、ユーザーが ON/OFF・並び替えできない状態でした。両方を通常の `simpleUi.sidebar` 項目として扱うよう変更:
...
旗鯖独自フォークとしては過去最大級の大型リリースです。フィードバック基盤 **HataFeed**、**気象庁発表の地震・津波情報ビューア**、**プライベートチャンネル**、Hatask ホームの全面リニューアル、Hataskey UI デッキの本格実装、ベータ機能 (C/C++ プレイグラウンド) などを同時投入しています。地震・津波情報ビューアは気象庁発表の情報をそのまま表示・通知する設計です (出典: 気象庁 / P2P地震情報、**緊急地震速報 EEW は非対応**)。気象業務法第17条・第23条に配慮し、旗鯖が独自に予報・警報・速報を発する形式は採っていません。あわせて、HataFeed 系エンドポイントへのレートリミット追加、絵文字申請 URL の危険プロトコル弾き、HataFeed イシューのカテゴリ書き換え攻撃防止など、複数のセキュリティ修正を行いました。本家 Misskey からの追加取り込みはありません。
...
- **Hataskey UI デッキ (`ui:simple` + `simpleUi.deckMode`) を本格実装**: 11.6 で導入した Hataskey UI デッキを、`profile → slots → frames → tabs` の 3 階層モデルで本格実装しました。タイル並び・列幅・タブ並び・タブ名・タブ色・タブごとの通知フィルタを編集でき、`profile` 単位で保存・切替できます (`simpleUi.deckProfilesV2`)
...
- **Hataskey UI サイドメニュー / 上部メニューの拡張**: 左サイドメニューの縮小時に**メニューチップ (ツールチップ) をホバーで表示**するようにしました。サイドメニューに「HataFeed」「地震・津波情報」「リロード」を追加し、上部メニューにも「リロード」を追加しています。また、Hataskey UI デッキ用のお知らせ吹き出しを個別に無効化できるトグルを追加しました
- **Hataskey UI 新機能案内ポップアップ (クリック非遷移)**: 「もっと！」から HataFeed と気象庁発表の地震・津波情報が確認できることを案内する吹き出しを追加しました。**気象業務法上の安全性に配慮し、クリックでは遷移せず案内のみ**を行います (端末ごと 1 回表示)
...
- **タイムライン日付セパレータの調整**: Hataskey UI デッキでは日付を詰めて表示し、スマホでは日付の表示/非表示をトグルできるようにしました (`simpleUi.showTimelineDateOnMobile`)。ノート間隔の `compact` を廃止し Misskey 準拠の挙動に揃えています
- **Hataskey UI デッキ最上部に「最新のノートです」案内**: 先頭のノートがタブバーに密着しないよう、最上部にメッセージとスペーサーを表示するようにしました
...
- **「もっと！」メニュー位置の案内**: Hataskey UI の「もっと！」案内アロー位置を、「もっと！」メニュー直下に表示するよう改善しました
...
- **新機能案内ポップアップの文言と非遷移化**: Hataskey UI の新機能案内ポップアップは「気象庁発表の情報を表示します」と明示し、クリックでは遷移しない案内のみとしました
...
マスコット機能の本格実装と、タイムラインの天気エフェクト、Hataskey UIでのデッキ表示対応という3つの大きな表現系機能を主軸とした、機能拡張・改善リリースです。マスコットは好きな画像を立ち絵として画面に表示し、表情・セリフ・モーション・通知連携・誕生日演出などを設定できる旗鯖独自機能です。天気エフェクトはノート本文の言葉に応じて雨・雪・日差し・強風・流れ星などの演出を背景に表示し、光過敏症に配慮して強い明滅を一切行わない設計としており、 Hataskey UI の表示モード切替やナビゲーション周りの拡張、多数のバグ修正・UI 調整も含みます。本家 Misskey からの追加取り込みはありません。
...
- **Hataskey UI の表示モード・ナビゲーション拡張**: デスクトップで通常表示とデッキ表示をワンタップで切り替えられるようにし、ナビゲーションを上部メニュー / 左サイドメニューから選べるようにしました。左サイドメニューは手動で縮小・拡大できます。デッキ表示中はノート間隔を「詰める」に強制して情報密度を上げ、Hataskey UI デッキ用のお知らせ吹き出しは個別に無効化できるトグルを追加しました
- **Hataskey UI のお知らせ吹き出しとサイドメニュー操作改善**: 新機能を案内するお知らせ吹き出しを追加し、サイドメニューの縮小/拡大ボタンを再配置しました
...
- **ページタイトル二重表示を解消するトグルを追加**: Hataskey UI でページタイトルが二重に表示される場合に解消できるトグル (`simpleUi.showPageHeader`) を追加しました
...
- **存在しない API 呼び出しによるクラッシュを修正**: ユーザーメニューのミュート/ブロック、および Hataskey UI の未読通知フォールバックが、存在しない `os.api` を呼び出してクラッシュする不具合をそれぞれ修正しました
- **削除済みメニュー項目が残る問題を修正**: 削除済みのメニュー項目 (`hataWhatsNew` 等) やサイドバー項目 (`whatsNew` 等) が、既存ユーザーのサイドメニュー・Hataskey UI ナビバーに残ってしまう問題を、描画時に除去するよう修正しました
...
- **Hataskey UI デッキのウィジェット枠線を修正**: Hataskey UI デッキでウィジェット枠線が反映されない問題を修正し、OFF 時には灰色枠を表示するようにしました
...
- **UI モード切替チップの文言変更**: Hataskey UI の UI モード切替チップの「シンプル表示」を「通常表示」に変更しました
...
- **機能解説 (旗鯖機能解説) を更新**: 「マスコット」カテゴリを新設し、天気エフェクト・Hataskey UI のデッキ表示・宴成功回数・登録申請インジケーターなど今回追加した機能の解説を追加しました
- **UI 選択モーダルの調整**: UI 選択モーダル (`MkUISetup`) で Misskey UI のカードを小型化し、「非推奨」バッジを表示するようにしました。旗鯖は多くの Misskey 機能をカスタマイズしているため、Hataskey UI の利用を推奨する意図です
...
- **外部アカウントの通知をリアルタイム受信**: 連携した外部サーバーの通知を旗鯖の画面上でリアルタイムに受け取れるようにしました。WebSocket による購読で新着通知をトースト表示し、専用の通知ページ (`/external-notifications`) で一覧を確認できます。外部TLタブや Hataskey UI のピル型ボタンからアクセスできます。あわせて、外部通知メニューの UI 統合・通知表示の改善・管理者保護機能を追加しています
...
hata-11.4 リリース直後に発見した複数のバグ修正と、Hataskey UI サイドメニューの体験改善、旗鯖の設定方針に基づく一部設定の固定化などをまとめたパッチリリースです。本家 Misskey からの追加取り込みはありません。
...
- **MisskeyUI / Deck UI のトレンドタブ位置を Hataskey UI と統一**: hata-11.4 で Hataskey UI のトレンドタブを通常タブの右端 (外部TLの左) に移動しましたが、MisskeyUI (`timeline.vue`) と Deck UI (`tl-column.vue`) のトレンドタブが左端のままだったため、すべてのUIで一貫した並び「通常タブ → トレンド → 外部TL」になるよう統一しました。MisskeyUI では `headerTabs` / `headerTabsWhenNotLogin` の配列順を変更してトレンドを通常タブ・リスト・アンテナ・チャンネルの後・外部TLの前に配置。Deck UI では従来トレンド列の表示用ロジック (`column.tl === 'trending'` の `v-else-if` など) は存在していたものの、列タイプ選択メニュー (`setType`) の `items` 配列に trending が含まれておらず**新規列として作れない**状態だったため、選択肢に追加して `bubble` の直後・`ohtl` の前に配置しました
- **Hataskey UI サイドメニューに表示/非表示トグルを追加**: サイドメニューの並び替え設定で、各項目を個別に表示/非表示に切り替えられるようにしました。トグルOFFの項目はサイドバーから非表示になり、並び替え画面ではグレーアウト (opacity: .45) で残ります (再表示可能)。タイムライン / 通知 / お知らせ / フォロー申請 / もっと の5つは必須項目として常に表示扱いとなり、トグルの代わりに鍵アイコン (`ti-lock`) が表示されてトグル不可になります (設定は元々サイドバー並び替え対象外)。既存ユーザー設定との互換性のため、`visible` フィールドが無い項目は表示 (`true`) として扱います。描画側 (`simple.vue`) でも `visible === false` の項目を除外する処理を入れていますが、必須項目はそこでも強制表示されるため、誤って `visible: false` で保存された場合の保険になっています
...
hata-11.3 でブランド統一・Misskey 2026.5.2〜5.4 取り込みという大きな整備を行った直後のリリースとして、利用者体験を細かく整える方向の更新になります。新機能としてメンテナンスお知らせ向けに**進捗バー (4段階)** を追加したほか、hata-11.3 の積み残しだった複数のバグ修正と Hataskey UI の体験改善を中心に取り込みました。本家 Misskey からの追加取り込みはありません (前リリースで 2026.5.4 まで追従済み)。
...
- **Hataskey UI サイドメニューの並び替えが画面に反映されない問題を修正**: `hata-custom.vue` の `moveArr` 関数が配列の分割代入 `[arr[idx], arr[ni]] = [arr[ni], arr[idx]]` で要素を入れ替えていましたが、Vue 3 のリアクティビティはインデックス直接代入を検知できないため、▲▼ボタンで並び替えても画面が再描画されませんでした。`splice` ベースの入れ替えに変更
- **Hataskey UI 設定の「トレンドタブを表示する」トグルがクリックで反応しない問題を修正**: `hata-custom.vue` の `showTrendingTab` computed が getter で非リアクティブな `prefer.s['simpleUi.showTrendingTab']` を参照していたため、Vue の依存追跡が効かず、トグルしても画面が再描画されず (リロードしないと反映されない) 状態でした。`prefer.r['simpleUi.showTrendingTab'].value` (リアクティブ ref) に変更し、即時反映するように
...
- **トレンドタブを通常タブの右端 (外部TLの左) に移動**: Hataskey UI 上部タブの並び順を「通常タブ... → トレンド → 外部ホーム → 外部ローカル」に変更しました。従来は `visibleTopTabs` でトレンドタブを配列先頭に固定していたのを末尾配置に変更し、外部TL (ohtl/oltl) は後段の `tabOrder` で `push` されるため、結果としてトレンドが「通常タブの右端、外部TLの左」の位置に収まります
- **MisskeyUI / Deck UI のトレンドタブ位置も同じ並びに統一**: 上記の Hataskey UI 変更に合わせて、MisskeyUI (`timeline.vue`) と Deck UI (`tl-column.vue`) のトレンドタブ位置も「通常タブ群の右端、外部TLの左」に統一しました。MisskeyUI では `headerTabs` / `headerTabsWhenNotLogin` の配列順を変更してトレンドを通常タブ・リスト・アンテナ・チャンネルの後・外部TLの前に。Deck UI では従来トレンド列の表示用ロジック (`column.tl === 'trending'` の v-else-if など) は存在していたものの、列タイプ選択メニュー (`setType`) の `items` 配列に trending が含まれておらず**新規列として作れない**状態だったため、選択肢に追加して `bubble` の直後・`ohtl` の前に配置しました
- **サイドメニュー並び替えをグループ内限定に + グループラベル表示**: Hataskey UI サイドメニューの並び替え設定で、従来はグループ (基本 / Hatask / 発見・交流 / もっと) をまたいで自由に動かせていましたが、表示側のグループ強制再分類との組み合わせでラベル位置が崩れる懸念があったため、**グループ枠を固定とし、並び替えは各グループ内でのみ可能**に変更しました。`canMoveSidebar(idx, dir)` で隣接要素のグループを比較し、別グループなら境界として ▲▼ ボタンを `disabled` に。並び替えリストにグループラベル (`isSidebarGroupHead` / `sidebarGroupLabelOf`) を表示し、どの項目がどのグループに属するか可視化しています
...
- **サイドメニューに「メッセージ」を追加 (既存ユーザー含む全員に反映)**: Hataskey UI サイドメニュー基本グループに「メッセージ」(`ti ti-messages`) を通知の直後に追加しました。`def.ts` のデフォルト変更だけでは保存済み `simpleUi.sidebar` を持つ既存ユーザーに反映されないため、`sidebarGroups` computed で「`sidebarOrder` に `chat` が無ければ基本グループの通知直後に動的注入する」方式 (トレンドタブと同じ手法) を採用しています。`def.ts` に `chat` がある新規ユーザーは `sidebarOrder` に持つため二重注入されません。クリック (`goToChat`) / 未読ドット (`hasUnreadChat`) / アクティブ判定 (`isChatPage`) は既存実装を流用
