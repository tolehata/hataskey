...
- **参加申請の連絡先**: すでに使っているSNSなどの連絡先を、任意で記入できるようにしました。用途は申請確認とそのための連絡に限り、承認・拒否のどちらでも申請データから自動削除します。登録用メールアドレスとは取り扱いが異なること、別途保管されたバックアップや控えは同時には消えないことも説明します。規約とルールは各サーバーの設定を使います
...
- **花常の提供終了**: ゲームの入口、機能紹介、関連する管理画面と専用APIを取り外しました。専用ソースと素材はリポジトリの保管用ディレクトリに残しますが、通常のビルド・配信には含めません。既存の保存データや過去のデータベース更新処理は削除しません
