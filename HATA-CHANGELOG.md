...
hata-12.7.1以降の更新です。お気に入りのフォルダ管理、Hatadyの画像添付と記録の整理、登録申請の共同審査を追加しました。気持ち記録のリマインダー、宴の参加判定と失敗時の演出、画面の更新や通知表示も改善しています。通常のHataskey UIには、お気に入り保存・絵文字追加・時報の案内を上部ナビバーへ加えました。

### お気に入りのフォルダ管理

- お気に入りの保存時に「未分類」またはフォルダを選べるようにしました。1つのノートは1つのフォルダへ保存し、あとから移動できます。既存のお気に入りは「未分類」としてそのまま引き継ぎます
- 通常のHataskey UIでお気に入りを保存すると、保存画面を閉じたあとに、ノートがフォルダへ入る演出と保存先の案内を上部ナビバーへ表示します。保存に失敗した場合や既存のお気に入りを移動した場合には、保存完了の演出を表示しません
- フォルダの名前・色を変更でき、ドラッグ＆ドロップで並び替えられます。PCは左側のナビゲーション、狭い画面はカプセル型のタブから切り替えます
- 子フォルダの作成はロールで制御し、既定では無効です。有効な場合は親・子の最大2階層で整理でき、ドラッグ＆ドロップで入れ子にしたり取り出したりできます。フォルダ数の上限もロールで設定し、親子の合計で既定2個・最大5個です
- フォルダや子フォルダを削除した場合は、中のお気に入りを「未分類」へ移します。親フォルダを削除した場合は、子フォルダ内のお気に入りも対象です。フォルダ削除によってお気に入りの登録や元のノートを削除しません
- Hataskey UIのデッキのお気に入りカラムに専用のフォルダ操作を追加しました。フォルダ管理をカラム上部へまとめ、並び替えの開始と並び順の保存、重複した見出しの表示を修正しました
- 作成上限に達した場合は新規作成ボタンを非表示にします。ロールの上限や子フォルダ権限が変わっても、作成済みのフォルダやお気に入りは削除しません
- 支援管理の「案内する特典」に、お気に入りフォルダ数と子フォルダ作成を追加しました。参照ロールとサーバー標準の比較、説明文、表示・非表示を設定できます。既存の設定を保持し、新項目は初期状態で非表示です。支援者へのロール自動付与は行いません

### Hatadyの画像添付・記録とコレクションの整理

- 活動記録と作品の鑑賞・プレイ記録へ、ドライブの画像を最大16枚添付できるようにしました。作成・編集・確認画面と下書きで画像を扱い、記録カード、詳細、スタッフの確認画面にも表示します
- 添付は本人の画像だけを受け付け、重複・枚数・ファイルの存在を確認します。削除済みのファイルは表示から除き、添付の閲覧には元の記録の公開範囲を適用します
- 記録の詳細、会話、プロフィール、検索結果などから本人の記録を削除する操作を揃えました。削除した記録は表示中の一覧・統計・連続記録へ反映し、取得途中の古い応答で再表示されることを防ぎます
- 本・作品のコレクションをまとめて確認できるようにし、種類の切り替えと検索を整理しました。本や作品を削除するときも、紐づく活動記録は残す既存の扱いを維持します
- 記録するカテゴリの選択、ホームとコレクションへの移動、スマートフォンで下へ引いて更新する操作を調整しました。プロフィールや連続記録の表示も、更新した内容に追従させます。リアクションの絵文字を押したときに、説明表示が操作を妨げる問題も修正しました

### 気持ち記録のリマインダー

- Hataskの画面を開いている間だけ動作していた通知を、サーバーから送る方式へ変更しました。画面を閉じていても、設定した時刻に当日の記録がなければ通常の通知へ届きます
- 通知時刻とタイムゾーンを保存し、既存設定と設定の書き出し・読み込みに対応しました。通知の送信前に設定と当日の記録を再確認します
- 同じ時刻の通知の重複を抑え、処理が遅れた場合も15分以内に限って補います。端末へのプッシュ通知は、従来どおり通知許可と購読設定に従います

### 登録受付とスタッフの共同審査

- 登録受付を「完全停止」「申請・招待」「公開登録」に分け、完全停止中は申請・招待・公開登録をサーバー側で拒否します。停止前の受付方式を保持し、再開時に復元します。外部へ公開する登録受付情報にも停止状態を反映します
- 登録申請に、管理者・モデレーターの賛成・反対と理由を記録する審査画面を追加しました。承認には確定時点で対象となるスタッフ全員の賛成が必要で、サーバー管理者が最終承認します。反対は保留として扱い、サーバー管理者が最終的な拒否を確定します
- 審査中に加わったスタッフは改めて投票対象とし、外れたスタッフの票は履歴として残します。古い審査状態での操作は競合として扱い、判定の上書きを防ぎます
- 申請者のSNSなどの連絡先は審査担当者が確認できます。メールアドレスはサーバー管理者だけに表示し、モデレーター向けの応答・検索・通知・管理ログには含めません。投票理由は最終裁定後に消去します
- 審査の通知、管理ログ、未処理件数とメニューの表示を更新しました。拒否済み申請の整理APIの登録漏れも修正しました

### 宴の参加判定

- LTL上で開く操作をせずに見える「宴」の宣言を参加対象にします。隠された本文、URLやリンク先などの表示されない文字、表示を隠せる装飾だけを使った参加を防ぎます。表示される宣言や従来の対象絵文字による参加は継続します
- 挑戦中の編集で宣言を隠した場合は失敗とし、本文を戻しても復活させません。本人の編集による失敗は他の人の阻止回数や実績へ加算しません
- 画面内で進行中の宴が失敗に切り替わったとき、ノートが割れて「失敗」を示し、元の表示へ戻る演出を追加しました。失敗済みのノートをあとから開いても再生しません。動きを減らす設定に従い、対象ノートの操作や画面幅の変更時には演出を終了します
- リアクション選択画面を閉じたときのフォーカス復元や自動スクロール、リアクション欄の高さの変化で、失敗演出が一瞬で終了する問題を修正しました。位置と高さの変化に追従し、本来の約1.65秒の演出を保ちます
- 成功を確定する前にもノートの状態を再確認し、画面の演出をサーバー側の判定へ揃えました。宴の処理で失敗しても、ノート編集の保存・履歴・通常の配送は継続します

### カスタム絵文字の追加・上部ナビバーの通知と時報

- 管理画面からカスタム絵文字を追加できない問題を修正しました。追加が完了した場合に、既存の絵文字追加通知へつなぎます
- サーバーに新しく追加された絵文字を、通常のHataskey UIの上部ナビバーで案内します。追加通知の重複や、名前変更による再表示を抑え、削除された絵文字の案内は取り下げます
- 端末の時刻に合わせ、毎時00分に時報を表示します。画面を閉じている間や非表示の間の案内は蓄積せず、復帰時に過去の時報をまとめて表示しません
- 「環境設定」から、絵文字追加通知と時報をそれぞれオン・オフにできます。既定はオンです。既存の上部ナビバーの表示順に統合し、通知一覧への保存やプッシュ通知は行いません

### その他の表示と操作

- Hataskey UIとデッキのタイムライン選択へロールの利用権限を反映しました。利用できない種類を選択候補から除き、保存済みのデッキ構成は削除しません
- 外部タイムライン連携の接続候補に「ゆうすきー」（`yugure.hata.blog`）を追加しました
- 通知許可ボタンを中央へ配置し、管理メニューの未処理表示をアイコンのそばへ揃えました
- Hataskの狭い画面でもスタッフ向けの記録確認へ移動できるようにしました。共通の下へ引いて更新する操作も、画面上端や処理中の状態に合わせて調整しました
- 共通フォントを複数画面から読み込んだ際に、本番ビルドが失敗する問題を修正しました
- アプリ内の「今回の更新内容」をhata-12.7.2へ更新しました。前版の開幕演出、改行とページ切り替え、小さい画面での分割、明暗テーマを維持し、今回の機能に合わせた画面例を用意しています

### 内部変更・互換性・更新時の注意

- 登録審査の投票、お気に入りフォルダの一覧・作成・変更・削除とお気に入りの移動をAPIへ追加しました。Hatadyの記録APIへ画像添付を加え、ロール・支援情報の定義とSDKの生成型も更新しています。本人確認、所有者の確認、スタッフ権限と第三者アプリの制限を各操作へ適用します
- マイグレーション`1789340000000-registration-closure-and-repository-default.js`で登録の完全停止設定を追加し、リポジトリURLの既定値をHataskeyへ更新します。管理者が独自に指定したURLは維持します
- マイグレーション`1789420000000-registration-unanimous-review.js`で登録申請の投票・審査版・最終判定を保存する列を追加します。既存の申請内容と状態は保持します
- マイグレーション`1789500000000-add-hatady-record-attachments.js`で活動記録と作品の記録に画像IDの配列を追加します。既存記録は添付なしとして扱い、本文や公開範囲は変更しません
- マイグレーション`1789580000000-add-note-favorite-folders.js`でフォルダのテーブルとお気に入りの所属先を追加します。既存のお気に入りのID・ノート・登録日時は保持し、所属先の空値を「未分類」として扱います
- **Hatadyの添付とお気に入りフォルダのマイグレーションは、自動ロールバックに対応しません。** 添付参照や分類情報の消失を防ぐため、`down()`は停止します。登録審査の`down()`は追加した審査履歴の列を削除するため、戻す前に履歴の保護が必要です
- 本番展開前に、件数を照合した本番DBのコピーで4本のマイグレーションを先行確認してください。今回の開発環境では既に適用済みのため、全体リビルド時に新たなマイグレーションを適用しません
- 気持ち記録のリマインダーを定期ジョブへ追加し、通知の保存と重複防止情報をRedisへ同時に記録します。これは同一通知の重複保存を防ぐ仕組みで、プッシュの完全な一回配送を保証するものではありません
- カスタム絵文字の追加時、DBで内部保存と確認できた画像は公開URLから取得し直さず、内部ファイルから独立したコピーを作ります。`cacheableLookup ENOTFOUND localhost`による失敗を回避し、既存の画像処理、実ファイルのサイズ上限、権限確認を維持します。外部URL・オブジェクトストレージの取得方法とアクセス制限は変更しません
- 絵文字のコピー元を整理する際、既存のノート・アバター・バナー・絵文字から参照されているファイルは削除しません。コピー失敗時も原本を保持します。今回の絵文字追加と宴の演出の修正に、DBマイグレーションやAPI形式の変更はありません
- ノート編集後に宴の判定を追加しています。ActivityPubのデータ形式・送信先・署名処理は変更せず、既存のUpdate配送を継続します。今回、外部連合先との実疎通は行っていません
- Node.js・依存パッケージ・外部素材の追加や更新はありません。既存の画面部品、テーマ、アイコン、書体を再利用し、本体のAGPL-3.0-onlyとSDKのMITを維持します

## hata-12.7.1

hata-12.7以降の更新です。Hatady・Hataskにスタッフ向けの記録確認を追加し、絵文字投票の表示設定、通知、HataFeedの報告、iPhone・iPadの表示、画面を戻る・閉じる操作を改善しました。

### 絵文字投票の表示と終了

- 共通の「環境設定 → タイムライン」からLTLの絵文字投票をオン・オフにできるようにしました。従来の設定値を引き継ぎ、設定検索にも対応します。オフにすると、Hataskey UIとデッキUIで投票の画面や演出を表示しません
- Hataskey UIの上部ナビバーに進行中の投票を表示し、参加を辞退したあとや結果を閉じたあとの状態を修正しました
- 受付時間を30秒とし、候補数を利用可能な絵文字の最大5件に揃え、投票の終了や期限切れ、画面を閉じたあとの表示を見直しました

### HataFeedの報告と改善予定

- イシュー作成に、使用端末・OSとバージョン・ブラウザや開き方の任意入力欄を追加しました。確認画面と端末の下書きへ引き継ぎ、報告の説明にまとめて送信します
- スタッフ向けの「改善予定を追加」をロードマップの上部に配置し、予定がある状態でも追加操作へ進めるようにしました

### 画面の表示・戻る操作・編集

- iPhone・iPadで上端の表示領域を調整し、ナビバー、ページ、メニュー、小窓、画像ビューアの配置へ反映しました
- ダイアログを閉じたあとに背面の画面を操作できなくなる問題を修正しました。ページ移動や小窓を閉じるときの後片付けも見直しています
- HataIntroの左上の矢印から、ガイド内の前のページへ戻れます。検索条件や選択項目も復元します。目次からは元の画面へ戻るか小窓を閉じ、戻る画面がない場合はホームへ移動します
- 通常デッキとHataskey UI内のデッキから、重複したウィジェット編集ボタンと余白を取り除きました。編集はカラムのメニューから行えます

- タイムライン上部とシンプルUIの常設投稿フォームで、画面を開いたときに入力欄へ自動でフォーカスしないようにしました

### スタッフ向けの記録確認

- **Hatadyの管理**: 本・作品・活動記録・コメント・リアクションを検索し、関連する前後の記録を確認できます。非公開記録に加え、本人用のしおりや内容メモも管理者・モデレーターの確認対象です
- **Hataskの記録確認**: 予定・ToDo・きもち・ごはん・おはなの記録を一覧・詳細で確認できます。種類、公開範囲、期間、確認状態などで絞り込めます。非公開記録と指定メンバー向けの記録もスタッフの確認対象です
- 確認状態を「未確認・要確認・確認済み」で共有します。Hatadyではスタッフ用メモも保存できます。確認者・確認日時と最新状態を保存する機能で、過去の操作履歴を蓄積するものではありません
- 記録の内容が変わると未確認扱いに戻し、古い内容や確認状態に対する更新は競合として扱います。確認状態は元の記録とは別に保存し、記録の本文や公開範囲を変更しません
- 画面とAPIは管理者・モデレーター専用です。第三者アプリのトークンによる利用は拒否します

### Hatask・Hatadyと更新案内

- Hataskの公開範囲メニューの位置とメンバー選択画面の重なりを修正しました。クイック入力のフォーカス枠も内側に収め、端で切れないようにしています
- Hatadyの一覧表示とページ切り替えで描画処理をまとめ、ページをめくる演出を保ちながら画面外の項目などの処理を抑えました
- Hatadyの通知は、画面を開いて読み込み・表示が成功したあとに既読にします。読み込み中に閉じた場合や取得失敗時には既読にせず、重複した既読操作や古い未読件数の応答による巻き戻りも防ぎます
- アプリ内の更新案内を、hata-12.7以降の変更だけを紹介する内容へ差し替えました。通常は6ページ、本文の高さが足りない場合は12ページに分けます。ページ数が変わっても読んでいた項目を保持します

### 内部変更・互換性・ライセンス

- 絵文字投票の状態と個別票をRedisから短時間のメモリ管理へ移しました。同一クラスタのHTTP workerはprimaryの状態を共有しますが、独立したprimaryやreplica間では共有しません。期限切れやprimaryの再起動で状態を破棄し、復元しません。投票のきっかけとなる通常ノートの保存は継続します
- 絵文字投票APIには`Cache-Control: no-store`を付け、アプリのHTTPアクセスログから除外しました。内部エラーの診断ログとtelemetryから投票パラメーターを除き、telemetryへの利用者IDの明示的な付加も止めました
- Hatady管理用の`hata/hatady/admin/moderation/{list,show,review}`、Hatask管理用の`admin/hatask/records/{list,show,review}`の計6エンドポイントを追加し、SDKの生成型を更新しました
- DBマイグレーション`1789250000000-add-hatady-moderation-review.js`と`1789260000000-add-hatask-record-review.js`で、確認状態を保存する専用テーブルを追加します。既存の記録本文は移行・削除しません。両マイグレーションの`down`は注記の消失を防ぐため停止する設計で、自動ロールバックには対応しません。本番展開前には、件数を照合したDBコピーで適用を検証してください
- ActivityPubのデータ形式・送受信・配送処理、Node.js・依存パッケージの更新は含みません
- 更新案内は既存のテーマ・アイコン・素材を再利用しています。新しい外部素材は追加せず、配布条件はAGPL-3.0-onlyを維持します

## hata-12.7

hata-12.6.3以降の更新です。Hatady・HataFeedの画面と記録操作を刷新し、お絵かきツールをHatadintとして拡張しました。Hataskのテーマ・予定共有・支援画面、機能紹介、通知、検索も更新しています。

### Hatadyの記録・コレクション・振り返り

- **記録の作成**: 「＋」から活動を選び、項目の入力、公開範囲の指定、内容の確認へ進む画面にしました。勉強・読書・映画・ゲームに加え、運動・作業を記録できます。時間は秒単位、開始時刻や複数タグ、活動ごとの詳細項目にも対応します
- **入力と下書き**: 時間入力、科目、タグ、公開範囲などの操作を整理しました。編集中に閉じるときは「端末に下書きを保存して閉じる」「保存せず閉じる」「編集に戻る」から選べます。下書きはアカウント別に端末へ保存し、サーバーの設定同期には含めません
- **ホームとコレクション**: 活動カード、本・映画・ゲーム・作業の一覧、作品詳細と記録詳細を作り直しました。読書のしおりやメモ、作品への感想、個々の記録へのコメント・リアクションを扱えます。作品を削除したあとも、その作品名などを記録側に残して履歴を振り返れます
- **公開範囲と管理操作**: 本・作品と個々の記録の公開範囲を分けて判定します。検索、一覧、詳細、コメント・リアクション、通知にも対象の閲覧条件を反映し、管理者・モデレーター向けの参照権限を通常の利用と区別しました。作業詳細の公開ログに紐づく非公開本のIDも、閲覧権限がない相手には返しません
- **プロフィール・統計**: プロフィールの色やカードの配置を設定できるようにしました。週ごとの記録、目標、継続状況、詳細統計をカードやグラフで表示します。目標と科目の編集、表示テーマの設定も新しい画面へまとめました
- **検索・通知・書き出し**: 活動の種類や日付による絞り込みを整理しました。通知は種類別の表示、削除と取り消しに対応します。記録の書き出しは対象範囲や形式を選ぶ画面に更新しました
- **使い方の案内**: 操作例つきのチュートリアルを追加し、設定から再表示できるようにしました

### HataFeedの報告・絵文字申請・管理

- ホーム、イシュー、ロードマップ、ベータ機能、通知、設定への移動を整理しました。ヘッダーから通知を確認でき、種類による絞り込みにも対応します
- イシューと絵文字申請は「＋」から順番に入力する画面へ変更しました。入力途中の内容は、保存するかを選んで端末の下書きへ残せます
- 絵文字申請の状態や審査履歴、スタッフのコメントを確認しやすくしました。スタッフは未処理の申請を一件ずつ続けて審査できます
- プロジェクトの作成・編集、書き出し、表示設定、チュートリアルを整理しました。管理・書き出しの操作は権限に応じて表示します
- ライト・ダーク・ペーパー・エスプレッソの表示テーマを追加し、選んだテーマを端末に保存します

### Hatadintのお絵かきと保存

- **制作画面**: お絵かきツールをHatadintとして刷新しました。PC・タブレットは左右の制作パネル、スマートフォンは下部の操作ボタンから利用できます
- **描画と編集**: ペン・鉛筆・マーカー・エアブラシ、筆圧、手ぶれ補正、図形、選択・変形、スポイト、レイヤーの追加・複製・合成に対応します。端末やドライブの画像を追加し、2本指でキャンバスを拡大・移動できます
- **作品の保存**: アカウント別の端末下書きの保存・復元、PNG書き出し、ドライブ保存、投稿への添付を用意しました。閉じるときは下書き保存・保存せず閉じる・編集に戻るを選べます。保存や復元に失敗した場合は編集を続けられます
- **初回の利用確認**: ドライブ保存・投稿添付の前に、利用規約を守ることへの同意を確認します。アカウントごとの同意日時と版を記録し、管理画面の同意管理にも表示します。Hatadint由来のアップロードはサーバー側でも同意を確認します

### Hataskのテーマ・ホーム・予定共有

- **共通の配置とテーマ**: 暁の配置を各テーマへ広げ、暁・苔・季・花信・刷・ハタキュの6テーマを用意しました。既存テーマの保存IDを引き継ぎ、本体テーマに合わせた明暗切り替えとプレビューを追加しました
- **ホームとお気に入り**: カレンダー、ToDo、日記、食事、お花など既存の画面への移動を整理しました。ホームに表示するお気に入りを選べます。旧「目」パネルと導入のお知らせを取り除き、設定やアプリ一覧の導線も更新しました
- **指定メンバーへの共有**: 予定を指定したメンバーだけに公開できます。予定の一覧・詳細、参加確認、締切のお知らせに同じ公開条件を適用します。公開先を変更できるのは作成者で、メンバーの組み合わせはテンプレートに保存できます
- **参加確認**: 辞退した招待を表示するか切り替えられるようにしました。カレンダーと予定編集の参加状態、予定の移動、クイック入力を共有範囲に合わせて更新しました
- **サーバー支援**: 支援先・特典・支援者と、自分への反映状況を確認する画面を追加しました。管理者は案内、特典、支援者の登録・解除を管理できます。登録は支援者表示用のフラグを更新し、決済やロールの自動付与は行いません。初期状態は無効です

### LTLの絵文字投票

- 自鯖の公開ノートで「絵文字を選ぶぞ」と投稿すると、LTLで絵文字投票を始められます。候補は自鯖の絵文字から最大5件を選び、閲覧注意・ロール限定の絵文字は除外します
- 受付時間は15秒で、1アカウントにつき1票です。集計後は票数と順位を表示します。他の人の個別票は返しません
- 閲覧者のLTL利用権限と元ノートの閲覧条件を確認します。削除・変更された絵文字や期限切れの投票を残さず、投票の開始に失敗してもノート投稿は成立させます

### 機能紹介・入口・検索

- **HataIntro**: Hataskの機能紹介へ、操作の見本、キーワード検索、詳しい機能解説を追加しました。ノート、予定、コレクション、チャンネル、HataFeed、制作ツール、設定などを画面例と一緒に確認できます
- **入口画面**: Hatask・Hatady・HataFeedの紹介とノート表示を更新しました。公開ノートの取得・表示処理を整理し、画面幅に応じて紹介や投稿の並びを調整します
- **検索**: 検索画面の分類や操作を整理しました。チャンネル検索は、参加済みかどうかや検索語の有無にかかわらず、非公開チャンネルを対象外にします
- **設定検索**: 新しいテーマ設定や表示設定、操作画面の構成に合わせて検索用の索引を更新しました
- **更新案内**: Hatask・Hatady・HataFeed・HataIntroの画面例と変更内容を紹介する案内へ刷新しました。小さいウィンドウではページを分割し、閉じた時点で確認済みの版を記録します

### 通知・ウィンドウ・ウィジェット

- Hatadyのデザインに合わせて通知を更新し、Hataskey内の通知表示も整理しました。外部通知や通常のトーストについて、表示位置と重なり方を調整しています
- ログイン実績のダイアログを更新し、次の実績までの日数と進捗を表示します。すべての実績を獲得した場合の表示も用意しました
- Hatady・HataFeedのダイアログ、見出し、入力部品、画面内ナビゲーションを共通化しました。ウィンドウの高さやスクロール、狭い画面での操作を調整しています
- 右ウィジェットバーの開閉と編集・追加操作を整理し、開閉状態を端末に保存します。通知ウィジェットの更新操作も修正しました
- リアクションミュート改善、Hatask導入、デッキ、メニュー縮小、HataFeedへの移動に関する古い案内を整理しました。リアクションミュート設定の移行処理は継続します

### 内部変更・更新時の注意・ライセンス

- Hatadyの記録・コメント更新・通知削除と復元、Hatadintの同意取得、Hataskの支援、絵文字投票のAPIを追加・拡張しました。対応するモデル、入力検証、認可、レート制限、SDKの生成コードと回帰テストも更新しています
- 新しいDBマイグレーションを4本含みます。Hatadint同意記録（`1788900000000`）、予定の公開範囲とメンバー索引（`1788990000000`）、支援設定と支援者フラグ（`1789080000000`）、Hatadyの記録項目・作品履歴・コメントと通知（`1789170000000`）を追加します
- Hatadyの移行では既存の分単位の時間を秒へ換算し、単一タグを複数タグへ引き継ぎます。作品削除後も記録を残せるよう外部キーを変更します。PostgreSQL 15以降の列指定付き`ON DELETE SET NULL`を使用します
- **Hatadyの記録拡張は自動ロールバックできません。** 新しい記録項目を失わないよう、`down()`はエラーで停止します。旧版へ戻す場合は、事前にデータを保護する変換手順が必要です
- 本番展開前に、本番DBのコピーで4本のマイグレーションを確認してください。コピーのリストア後には元DBと件数を照合し、移行後の既存記録と公開範囲を確認してください
- ActivityPubのデータ形式・配送処理は変更していません。ノート作成処理にローカル絵文字投票の開始判定を追加していますが、投票と支援情報を新たなActivityPubデータとして連合配信する変更はありません
- Node.js・依存パッケージ・外部素材ファイルの追加や更新はありません。既存素材を再利用し、配布条件はAGPL-3.0-onlyを維持します

## hata-12.6.3

hata-12.6.2からの修正です。

- 非ログイン時に、画面の上半分へ「読み込みに失敗しました」が表示され、ログイン画面が崩れる問題を修正しました
- サーバーの既定テーマを保存するとき、設定の比較処理が`null`をオブジェクトとして扱って例外になる原因を修正しました。保存済みのテーマやほかの設定、保存形式は維持します
- ライト・ダーク両方のテーマについて、非ログイン状態での保存・再読み込み・初期値への戻しと、ほかの設定の保持を確かめる回帰テストを追加しました
- API、データベースマイグレーション、ActivityPubの形式・配送処理、Node.js、依存パッケージの変更はありません。外部素材の追加はなく、配布条件はAGPL-3.0-onlyを維持します

## hata-12.6.2

hata-12.6.1以降の更新です。Hataskにランキングを追加し、通知設定の保存と同期、登録申請、ミュートの反映、入口画面の表示を改善しました。

### Hataskのランキング

- **4種類の記録**: お花の数・宴の成功数・宴の阻止数・ログイン日数を、今月・今週・今日の期間別に表示します。同じ件数は同順位とし、前回の集計からの順位差も確認できます。集計は1時間ごとに更新され、日付はサーバーの時刻に従います
- **参加設定と公開範囲**: 初期状態は参加で、ランキング画面からいつでも不参加に変更できます。プロフィールで非表示にした花数・宴の記録と、非公開の花ギャラリーは表示しません。宴は公開ノートの記録が対象です。ログイン日数の表示はランキングへの参加設定に従います
- **表示対象**: 自鯖のアカウントを対象とし、凍結・削除・不参加の人、閲覧者のミュート対象、双方向のブロック相手を順位・参加人数・ページ計算から除外します
- **入口と実績通知**: Hataskの各テーマと暁のアプリ一覧へ接続しました。暁の下部ナビにも追加できます。ランキング画面の宴の実績通知は、Hatask設定からオフにできます

### 通知フィルターの保存と同期

- 通知画面のBOT除外設定をアカウントごとに保存します。ウィジェットの通知フィルターは確定時に保存し、旧旗鯖デッキからの移行でも設定を引き継ぎます
- 別タブやクラウドから届いた設定を取り込み、未保存の編集と統合するようにしました。設定画面を開いている間の同期や、標準デッキの次の編集で、保存済みの内容を古い値へ戻す問題を修正しました
- 設定検索の検査対象へ、BOT除外設定とランキングの実績通知設定を反映しました

### 登録申請・ミュート・入口画面

- **管理者との関係申告**: 参加申請に「サーバー管理者と関係がある」のチェックを追加しました。チェックした場合は理由欄を無効にし、SNSなどの連絡先を必須にします。入力途中の理由は送信せず、審査記録には本人申告であることを明記します。チェックしない場合は、理由必須・追加連絡先任意のままです
- **審査の継続**: 関係の申告による自動承認は行いません。メールアドレスやCAPTCHAなどの検査と、管理者による審査を継続します。連絡先は承認・拒否の確定時に申請データから削除します
- **モデレーターのミュート**: ミュートしたモデレーターのリアクションも非表示にします。操作直後と一覧の再取得後で同じ条件を使います。モデレーターへのブロックは引き続き利用でき、サーバー管理者へのミュート・ブロック禁止も維持します
- **入口画面**: 不要な横スクロールを抑え、縦方向のスクロールを維持しました

### 内部変更・更新時の注意

- ランキングの取得・参加設定APIを追加し、登録申請APIへ関係申告の項目と連絡先の必須条件を追加しました。SDKは更新後のAPI定義から再生成しています
- 新しいデータベースマイグレーションを1本含みます。`user_profile.hataskRankingParticipating`列を初期値`true`で追加し、`utage_session`の集計用索引を追加します。既存アカウントも初期状態はランキングに参加します
- 本番展開前に、本番DBのコピーでマイグレーションを検証してください
- ActivityPubの形式と配送処理は変更していません。ランキングは既存記録のローカル集計で、ランキング情報を新たに連合配信しません
- Node.js・依存パッケージ・外部素材の追加や更新はありません。配布条件はAGPL-3.0-onlyを維持します
- アプリ内の更新案内はhata-12.6.2の変更を日本語・英語・簡体字中国語で紹介します。過去の版の履歴は、このチェンジログに残しています

## hata-12.6.1

hata-12.6以降の更新です。Hataskのお花の一覧・詳細と自動スクロールを改善し、Safariの花壇表示、暁のダーク表示、Hataskey UIの通知と上部ナビを修正しました。

### お花の一覧と花壇

- **一覧ポップアップ**: みんなのお花・フラワーギャラリーそれぞれに、一時停止・再開ボタンの左から開ける一覧を追加しました。並び替えとページ切り替えを一覧内へ移し、自動スクロール画面の操作を整理しました。一覧からも個別の花の詳細を開けます
- **花名とアカウント表示**: サムネイルの花名を2行まで表示します。自分のフラワーギャラリーでは、花ごとに繰り返していた自分のアカウント名を省き、アイコンとアイコンデコレーションは残しました
- **詳細ポップアップ**: 花をタップ・クリックした直後のスクロールによって、詳細が表示前に閉じる問題を修正しました。一覧から開いた詳細を閉じると元の花へ操作位置を戻し、通報画面へ進む際は一覧を閉じてから表示します
- **自動スクロール**: ブラウザによるスクロール位置の丸めで動きが止まる問題と、細かくがたつく問題を修正しました。ドラッグ・スワイプ、停止・再開、アニメーション設定に応じた停止は維持します
- **Safariの花壇表示**: iOS SafariのPWAで葉や茎だけが表示され、花が表示されない問題に対応しました。花の描画をSVGの画像要素へ変更し、既存の絵文字スタイルと画像の読み込みに失敗した場合の代替表示を維持します
- **公開範囲の反映**: 一覧ポップアップにも既存の公開範囲を反映します。ミュート・ブロックなどの変更時は、開いているみんなのお花の一覧を閉じ、古い取得結果が再表示されないようにします

### アプリ一覧・通知・上部ナビ

- **暁のダーク表示**: Hatask App・Hataskey Appの機能カードが灰色にくすむ問題を修正し、暁の背景色へ揃えました
- **通知の閉じ残り**: Hataskey UIでタッチ操作やPWAへの復帰後などに通知が閉じず残る問題を修正しました。ほかのポップアップを開いていても通知の表示時間は進み、マウスを重ねたときやキーボードで通知を操作している間は一時停止します
- **スマートフォンの上部ナビ**: アカウントアイコンを上部ナビ内の左端へ収めました。タブを横スクロールしてもアイコンの位置を保ち、新着ノートの案内はタブの下へ表示します

### 内部変更・互換性・ライセンス

- お花の詳細・一覧・描画と、通知の表示時間・閉じ処理・高さの再計測を確かめる回帰テストを追加しました
- この版では新しいAPI、データベースマイグレーション、ActivityPubの送受信処理の変更、Node.jsの更新を含みません。保存済みのお花・予定・ToDoと既存のテーマ設定を維持します
- アプリ内の「今回の更新内容」はhata-12.6.1の変更を日本語・英語・簡体字中国語で紹介します。過去の版の履歴は、このチェンジログに残しています
- 依存パッケージと外部素材の追加はありません。配布条件はAGPL-3.0-onlyを維持します

## hata-12.6

hata-12.5.4以降の更新です。Hataskのお花を眺める画面と満開通知を追加し、暁ホーム、Hataskey UIの通知、新着ノートの案内を改善しました。

### お花の成長とギャラリー

- **お花の画面**: 育成中のお花、残り時間、花言葉をまとめて表示し、自分のお花とみんなのお花を流れる一覧へ変更しました。選んだ花の詳細から、名前の変更へ進めます。他の人の花には育てた人と収穫日時を表示し、通報の操作も維持しています
- **みんなの花壇**: 公開範囲に従って取得したお花を花壇に並べます。ブロック・解除後は、みんなのお花の表示も更新します
- **スクロールと操作**: お花の一覧はドラッグやスワイプでも操作できます。自動スクロールの停止・再開に対応し、操作中、画面を離れたとき、アニメーション無効時や端末の「動きを減らす」が有効なときは動きを止めます
- **季節とレア花**: 春夏秋冬それぞれ4品種、計16品種を追加しました。日本時間の季節に合わせて、出会いやすい品種を変えました。季節外の品種も出現します。これから育てるレア花の開花時間は約48〜96時間、通常のお花は約8〜32時間です。育成中のお花の残り時間と収穫済みのお花は維持します
- **収穫と名前の入力**: おはな・EYEのどちらからも収穫でき、入力中に画面や対象の花が変わった場合は保存を中止します。読み込みに失敗した状態での保存や二重入力も防ぎます
- **満開通知**: アプリを閉じている間に満開になったお花も、通常通知とWeb Pushでお知らせします。通知の種類別フィルターと受信設定に「Hataskのお花」を追加しました

### 暁ホームとアプリ一覧

- **いま使いたいもの**: 予定・ToDo・ごはん・HataFeedの通知から、状況に合う内容をホームへ表示します。利用者が表示内容を選ぶこともできます
- **利用頻度の反映**: スマートフォンのホームによく使うツールを並べるため、端末内でアカウントごとに利用状況を記録します。予定・ToDoなどの保存先は変更しません
- **表示の整理**: きょうの時間帯別の予定を開閉できるようにし、ホームの表示設定を各項目へ反映します。Hataskey Appの見出しと「ツール」の間にあった余分な空白も取り除きました

### Hataskey UIの通知と新着ノート

- **通知の自動配置**: 通常通知・外部通知を、PCでは上部ナビの下、モバイルでは上に表示します。モバイルでタイムライン以外を開いているときは、画面上部に通知だけを表示します。PCで上部ナビを隠した場合や、PCのデッキ表示では右下へ配置します
- **表示時間と並び方**: Hataskey UIの通知は5秒で閉じ、残り時間をリングで表示します。上部では新しい通知へ入れ替わり、PCの右下では最大3件を縦に並べます。ほかのUIでは従来の位置・並び方の設定を使います
- **本文・絵文字・リアクション**: 通知本文の改行や折り返し、カスタム絵文字、リアクションの見せ方を改善しました。通知一覧とポップアップで共通の本文処理を使います
- **新着ノートの案内**: 通常・外部・トレンド・リスト・アンテナの新着表示を上部ナビへまとめました。新着ノートへ進む操作は、それぞれのタイムラインへ引き継ぎます
- **ノートの名前とID**: アイコン未設定のロールが名前と`@ID`の間に空欄を作る問題を修正しました。通常表示と詳細表示で、画像のあるロールだけを描画します

### メディア・HataFeed・登録時の修正

- **リモート動画**: 部分取得やサムネイルの応答で、本文と`Content-Length`が食い違う問題を修正しました
- **無音動画の判定**: 再生失敗・中断・判定のタイムアウトを無音の根拠として扱わないようにし、音声がある動画までループ表示になる場合がある問題を修正しました
- **HataFeedのページ切り替え**: 1ページ100件の表示と次ページの取得を修正し、条件を切り替えたあとに古い取得結果が表示を上書きしないようにしました
- **登録申請のID**: 入力した大文字・小文字を承認後も保持します。大小文字だけが異なるIDで重複申請することはできません。既存アカウントのIDや申請済みの文字列は書き換えません
- **リアルタイムモード**: 設定の保存が完了してから再読み込みするようにしました
- **タッチ操作**: 既定のスクロールを妨げないタッチ開始イベントを、各ヘッダー・アバター・メディア表示などで明示しました

### 更新時の注意・互換性・ライセンス

- **データベース**: `1788700000000-preserve-registration-username-case.js`を追加しました。審査待ちの登録申請に使う一意索引を`LOWER(username)`へ置き換えます。審査待ちに大小文字だけ異なるIDがある場合は適用に失敗するため、更新前に重複を確認してください。本番適用前にはDBのコピーで移行を先行確認し、復元後のユーザー・ノート件数を元DBと照合してください
- **通知の内部処理**: 既存のシステムキューで満開を定期確認します。所有者の確認と通常の重複抑止を行い、SDKとサービスワーカーにも新しい通知種別を反映しました
- ActivityPubの送受信処理とNode.jsのメジャーバージョンは変更していません。既存のテーマ、予定・ToDoの保存先は維持します
- アプリ内の「今回の更新内容」はhata-12.6だけを特集します。過去の版の履歴は、このチェンジログに残しています
- 外部素材と依存パッケージの追加はありません。配布条件はAGPL-3.0-onlyを維持します

## hata-12.5.4

hata-12.5.3以降の更新です。[本家Misskey 2026.9.0](https://github.com/misskey-dev/misskey/releases/tag/2026.9.0)からの取り込みと、それをHataskeyの独自機能へ組み込むための修正・調整を含みます。

### 本家Misskeyからの取り込み

- **セキュリティ**: 本家Misskeyのセキュリティ修正を取り込みました
- **音声ビューアー**: 音声プレイヤーを画像・動画ビューアーへ統合し、再生位置・音量の操作と音声ビジュアライザーを追加しました
- **画像の拡大とぼかし**: ピクセルアート向けの拡大表示を追加しました。メディア一覧で解除したぼかしは、ビューアーでも解除した状態を引き継ぎます。Blurhashが表示されない場合がある問題も修正しました
- **リアクション**: 「利用できるリアクションを先頭に表示」の判定と並び順を修正しました
- **通知トースト**: 左側へ置いた通知が右側から出てくる問題と、アニメーションの挙動を修正しました
- **操作と応答**: スマートフォンで一部の文字を選択できない問題、管理画面のメール検索でエラーが二重に表示される問題、リストの人数が追加・削除後に更新されない問題を修正しました。設定検索とワードミュートの処理も改善しています
- **実験的設定**: 「Enable condensed line」のスイッチを削除しました
- **サーバー処理**: スレッドを重複してミュートした際のエラー、一部のWebSocketチャンネルの後片付け、古いアカウントで連合できない場合がある問題を修正しました。パスワード変更先を示す`/.well-known/change-password`にも対応しています
- **依存関係と翻訳**: pnpm 11.25.0、Vue 3.5.42、Vite 8.2.2などへの更新、ペルシャ語の追加、各言語の翻訳更新を取り込みました

### 本家の仕様変更に伴う注意点

- **連携アプリのリアクション取得**: `notes/reactions`はGETからPOSTへ変更する必要があります
- **絵文字の一括インポート**: ロールポリシーの有無にかかわらず、管理者だけが実行できるようになりました
- **連携アプリの接続解除**: 使用中のアクセストークン自身を、連携アプリから失効できるようになりました

### Hataskeyでの修正・調整

- **独自機能への適用**: 本家のセキュリティ修正を、Hataskey独自の構成にも適用しました
- **メディアの表示と操作**: 本家の新しいビューアーを、Hataskeyの配色、アニメーション設定、端末の「動きを減らす」に合わせて調整しました。再生・シーク・音量などの操作には読み上げ用のラベルを補っています
- **リアクションミュートとの両立**: 本家の並び順の修正を、Hataskeyのミュート処理後の一覧へ適用しました。独自のリアクション一覧画面もPOSTでの取得へ変更し、ミュート状態と表示更新時の再取得制御を維持しています
- **通知・メディアのスタイル**: 本家の表示変更をHataskeyのCSS構成に合わせて組み込み、通知の表示位置やピクセルアート向けの拡大設定が反映されるよう調整しました
- **保存済み設定との互換性**: 廃止した「Enable condensed line」は設定画面と設定検索から外し、保存済みプロファイルを読むための内部定義は残しました。Hataskeyでは更新前から表示処理に使われていなかった設定です
- **ライブラリ更新への対応**: Hataskey側のキュー処理とファイル名の扱いを更新後のライブラリへ合わせました。SDKはHataskey自身のAPI定義から再生成し、本体とSDKの`basedMisskeyVersion`を2026.9.0へ揃えています
- **独自の文言と更新案内**: Hataskey独自の翻訳を保持し、追加言語の名称表記もHataskeyへ揃えました。アプリ内の更新案内は日本語・英語・簡体字中国語で更新し、本家の改善とHataskey向けの調整を明記しています

### 更新時の注意・ライセンス

- 新しいデータベースマイグレーションとNode.jsのメジャー更新は含みません。既存の予定・ToDo・テーマ設定の保存先は変更していません
- Hataskeyで提供を終了したノート翻訳機能は復活させていません。本家のCW翻訳の変更は今回の取り込み対象外です
- 追加した`@zip.js/zip.js`のライセンスはBSD-3-Clauseです。Hataskey全体の配布条件はAGPL-3.0-onlyを維持します
- 更新案内の4タブにはhata-12.5.4・hata-12.5.3・hata-12.5.2・hata-12.5を表示します。hata-12.5.1以前の履歴もこのチェンジログに残しています

## hata-12.5.3

hata-12.5.2以降の更新です。Hataskへ新しい標準デザイン「暁」を適用し、予定・ToDo・日々の記録を見渡せる画面へ刷新しました。カレンダーの詳細表示と空き枠からの操作を追加し、検索、特集カード、スマートフォンのメニューも整えました。

### Hataskの新デザイン「暁」

- **新しい標準テーマ**: 初めて使う場合のテーマを「暁」にしました。これまでの「季・花信・刷・ハタキュ」と保存済みのテーマ選択は残し、設定から切り替えられます
- **暁の紹介ポップアップ**: まだ紹介を見ていない利用者へ、Hataskを開いたときに「Hatask V3 Akatsuki」を案内します。その場で暁へ切り替えるか、今のテーマを使い続けるかを選べます。初回チュートリアルとは重ねず、設定を読み込めなかった場合は案内から上書きしません
- **扉をくぐる文字アニメーション**: Righteousの「Hatask」から「Akatsuki」へ、扉をくぐるように文字が切り替わります。紹介を閉じたときや、アニメーションを無効にしたとき、端末の「動きを減らす」を有効にしたときは演出を止めます
- **PCの3ペイン**: サイドメニュー、メイン画面、記録やToDoを置く右ペインを組み合わせました。右ペインの各項目は角を大きく丸めたケースで揃え、スクロール部分の上下を滑らかにぼかします。縦長のタブレット表示では、ホーム以外の右ペインを隠します
- **テーマに沿う背景**: 選んだテーマの配色を使い、時間帯に合わせてグラデーションが緩やかに変化します。カテゴリの切り替えにもアニメーションを付け、アプリのアニメーション設定と端末の「動きを減らす」に対応しました
- **見出しと操作列**: ハンバーガーメニューとサイドメニューのアイコン位置を揃え、右隣にHataskのロゴを配置しました。ホームの次の予定は文節に合わせて改行し、通知バッジの数字は白色で表示します
- **各タブの表示**: カレンダー・ToDo・お花・EYEを暁の画面へ接続し、きもち・ごはんの見出しや説明、カレンダー詳細設定のトグルへテーマ色を適用しました。テーマ選択欄の下端の見切れと説明文の改行も修正しています

### アプリ紹介と検索

- **Hatask App・Hataskey App**: 機能ごとのカードと外枠を統一しました。
- **特集バナー**: PC・モバイルとも特集の高さを抑えました。
- **ヘッダー内の検索**: 暁では、入力したあとに別の検索欄を開かず、元の入力欄の下へ結果を表示します。従来テーマの検索画面は維持します。
- **みんなのお花**: 暁の並び替えをN（New・新しい順）とO（Old・古い順）で表示します。自分のお花は従来の並び替え名を使い、PCの非選択ボタンをアイコンのみ表示します。従来テーマの並び替え表示は維持します

### カレンダーと日々の記録

- **予定の詳細**: 予定を選ぶと、PCでは日付の上または下の空きに合わせた吹き出し、モバイルではモーダルで詳細を表示します。画面の高さに収まらない内容は中でスクロールでき、編集や参加の操作は既存の権限に従います
- **空き枠から作成・コピー・移動**: 予定が入っていない場所を選ぶと、実行したい操作を選べます。PCは吹き出し、モバイルはモーダルで表示し、入力中の内容がある場合は破棄を確認します
- コピーした予定は新しいIDの非公開予定として作成し、参加者や公開先へ自動送信しません。元の予定は残します。公開予定の移動は、既存の公開同期処理を使います
- きもち・カレンダーの通知から対応するタブへ進めるよう、通知のリンクを画面の入口へ接続しました。記録欄に入力中の内容がある場合は保ちます

### ナビゲーション・メニュー・設定

- **スマートフォンの下部タブ**: 暁の設定で、各項目のプルダウンメニューとドラッグによる並び替えを使えます。同じ項目の重複を防ぎ、ホームとHatask Appは表示を必須にして位置だけを変更できます。この設定は暁を選んでいるときだけ表示します
- **子メニューの見切れ**: 下部タブの「位置を変更」やノートのユーザーメニューなど、さらに分岐するメニューが左右に見切れる問題を修正しました。表示領域に合わせて位置と幅を調整します
- **デッキでの終了**: 「Hataskを終了」で窓の中のタイムラインへ移動してしまう問題を修正し、Hataskの窓を閉じるようにしました。通常ページでは従来の戻る操作を使います
- 設定の右ペインへ埋め込んだ移行画面で、ホイール操作が外側へ伝わらない問題を修正しました。設定検索は開くたびに前面の表示順を取得し、デッキの窓の背後へ隠れないようにしています
- 設定プロフィールの管理画面へ、既存のプロフィール操作メニューを戻しました。設定検索の索引と検査も、追加された操作に合わせて更新しています

### Hataskey UIの表示修正

- **お知らせバナー**: バナー表示に設定したお知らせが画面上部へ出ない問題を修正しました。バナーの高さに合わせてナビゲーションの位置も調整します
- **折りたたみ端末の上部ナビ**: 画面全体ではなくタイムライン列の中央上部へ配置し、ウィジェット列の境界と重ならないようにしました
- **未連携時の外部通知**: 縮小サイドメニューとデッキ表示で、外部アカウントを連携していないのに外部通知が表示される問題を修正しました。保存した位置は変えず、連携を再開すると元の位置へ表示します

### 素材・保存データ・互換性について

- **フォント**: Archivoの可変WOFF2を自己ホストし、OFL-1.1のライセンス全文、著作権表示、配布元とハッシュを同梱しました。Hataskの利用時にGoogle Fontsへ接続する必要はありません。ロゴには既存のRighteousを使用します
- この更新はフロントエンドの表示と操作の変更です。新しいバックエンドAPI、データベースマイグレーション、Node.jsのメジャー更新は含みません
- ActivityPubの送受信処理は変更していません。既存の予定・ToDo・記録を保持し、保存先の読み込み失敗時に空のデータで上書きしない保護と、更新の競合を検出する仕組みも維持しています
- アプリ内の更新案内とREADMEをhata-12.5.3へ更新しました。更新案内は日本語・英語・簡体字中国語を揃え、抜けていたhata-12.5.1の案内を復元しました。hata-12.5.3・hata-12.5.2・hata-12.5.1・hata-12.5の4タブを、PCでは横一列、狭い画面では2列で表示します

## hata-12.5.2

hata-12.5.1以降の更新です。宴の成功・阻止回数に応じた実績を追加し、外部通知と外部タイムラインをHataskey UIへなじむ形に整えました。あわせて、同じサーバーのみで楽しめるタイムラインの隠し演出を追加しました。

### 宴と実績

- **宴の成功実績**: 成功10回の「いい感じ」から100回の「前代未聞」まで、10回刻みで10個の実績を追加しました。30～80回には宴ブロンズ・シルバー・ゴールド・ダイヤモンド・プラチナ・マイスターが含まれます
- **宴の阻止実績**: 阻止10回の「慣れてきた？」から100回の「宴ブロッカー」まで、10回刻みで10個の実績を追加しました。「隙がない」「TLの番人」「いつ寝てるの？」など、到達回数ごとに異なる実績を獲得できます
- **早すぎる宴の終わり**: ほかの人が始めた宴を開始から5秒以内に阻止すると、特別な実績を獲得します。この実績は本更新後の阻止から記録します
- **既存回数の反映**: データベースに残っている公開ノートに紐づく宴の成功・阻止回数を集計し、すでに到達している回数実績を自動で補います
- 宴実績はサーバーで確定した記録からのみ解除し、クライアントから自己申告して解除するAPIの対象には含めません。他人の実績一覧では、成功回数・阻止回数それぞれの公開設定を反映します

### 外部通知とサイドメニュー

- **外部通知の並び替え**: 外部通知を固定位置へ差し込む方式から、保存済みのサイドバー項目として扱う方式へ変更しました。通常のサイドバー編集とHataSideStudioで、ほかの項目と一緒にドラッグして並び替えられます
- 通常のサイドバー編集はこれまでどおりドラッグ操作だけを使います。外部通知を削除・非表示にする操作は追加していません
- 既存のサイドバーでは通知の直後へ外部通知を1件だけ補い、現在の並び順・表示設定・グループを保ちます。移行の完了はアカウントと設定プロフィールごとに記録し、保存に失敗した場合は次回起動時に再試行します
- HataSideStudioの保存形式を更新し、既存の全プロフィールの拡大・縮小メニューへ外部通知を補います。外部アカウントとの連携を外している間は表示だけを隠し、保存した位置は変更しません

### 外部タイムライン

- **Hataskey UIに合わせた表示**: Hataskey UIの通常表示とデッキ表示で、外部ノートのカード・余白・背景・操作列を本体のノートに合わせました。アバターもノートの外枠内へ収めています。従来UIとHataSNSCordUIの表示は変更していません
- **返信・引用・リノート**: 返信先をアバターと本文が一体になった表示へ変更しました。純リノートが何段続いても元ノートを一度だけ表示し、引用は引用として残します。循環したデータや深すぎる連鎖では再帰表示を安全に止めます
- **リアクション**: 外部サーバーでの操作に成功したリアクションを、リロードせずその場で表示します。絵文字URLが後から届いた場合も画像へ更新します。表示できないカスタム絵文字がカラーバーになる問題を修正し、リアクションの利用者名でもカスタム絵文字を表示できるようにしました
- **チャンネルノート**: 外部タイムライン上でもチャンネルのアイコンと名前を表示し、通常のノートと見分けられるようにしました

### タイムラインの隠し演出

- 本文が「TL崩れる」と完全一致する公開・ローカル・非チャンネルのノートを投稿すると、同じサーバーでHataskey UIを利用している画面へ演出を配信します
- 設定ボタン、投稿フォーム、ナビゲーション、表示中のノートなどが上から順に崩れ、10秒後に元の位置へ戻ります。入力内容や画面の状態は変更せず、見た目だけを一時的に動かします
- 1人につきサーバーのUTC日付ごとに2回まで発動できます。演出中はサーバー全体で重ねて発動せず、その間の投稿は日次回数を消費しません
- アニメーションを無効にしている場合と、端末で「動きを減らす」を有効にしている場合は再生しません。再生中に設定が変わった場合もすぐ元へ戻します
- 演出の配信には投稿者・ノートID・本文を含めません。判定用Redisに障害が起きても、成立した投稿そのものは失敗させません

### 表示と更新案内

- Hataskey UIのサイドメニューで、通知・外部通知・お知らせの未読バッジをメニューボタンの縦中央へ揃えました
- 「今回の更新内容」に「最新のリリース」を新設し、hata-12.5.2・hata-12.5.1・hata-12.5の3つを切り替えて確認できるようにしました。日本語・英語・簡体字中国語の案内を揃えています
- READMEの最新リリース表記をhata-12.5.2へ更新しました

### 連合・保存データ・互換性について

- `utage_session.interruptedWithin5Seconds`列を追加し、既存プロフィールへ到達済みの宴回数実績を追記するデータベース更新を含みます。保存済みデータを削除する更新ではありませんが、本番適用前に本番データベースのコピーで移行を先行確認し、復元後の利用者数・ノート数・宴セッション数・実績の付与結果を照合する必要があります
- 既存データからは公開ノートに紐づく成功・阻止回数だけを反映します。5秒以内の阻止を記録する列は既存行を`false`として追加するため、「早すぎる宴の終わり」を過去の記録から付与することはありません
- 実績型と同じサーバー内で使う演出通知の型をSDKへ反映しました。ActivityPubの配送・受信・HTTP署名方式は変更していません。「TL崩れる」のノート自体は、通常の公開ノートとしてこれまでどおり連合へ配送されます
- 新しい依存ライブラリや外部素材、ライセンス文書は追加していません

## hata-12.5.1

hata-12.5の修正版です。投稿まわりの案内と待機表示、Hataskのハタキュテーマ、ログイン画面の日本語サーバー名表示を整え、外部アカウントの接続先を追加しました。

### 表示と操作の改善

- **Hataskのスマートフォン表示**: ハタキュテーマでToDoの残件案内が二重に表示される問題を修正しました。きもち・ごはんでは、見出しと背景が同じ色にならないよう配色を整えました
- **投稿完了の案内**: 通常の投稿後に表示していた上部の「ノートを投稿しました」を取り除きました。返信・引用・編集の完了案内と送信完了の動きはこれまでどおり表示します
- **投稿待機の表示**: 通常の投稿フォームで送信を待っているカプセルを、フォームの外側にあるすぐ下の位置へ移しました。ほかの画面に埋め込まれた投稿フォームの配置は変更していません
- **日本語を含むサーバー名**: ログイン画面の左上で、漢字・ひらがな・カタカナを含むサーバー名の文字が粗く見える問題を修正しました

### 外部アカウント連携

- **Juice Serverを追加**: 外部アカウント連携の接続先にJuice Server（mk-juice.dev）を追加しました。接続先では、そのサーバーの規約が適用されます

### 更新案内と文書

- 「今回の更新内容」の先頭で「このリリース」と「このメインリリース」を切り替えられるようにしました。hata-12.5.1の修正内容と、hata-12.5の主な変更を分けて確認できます
- READMEの概要文を簡潔にし、Hataskeyが使用されているインスタンスの見出しを整理しました

## hata-12.5

Hataskの予定・ToDoを大きく見直し、みんなのお花を眺める機能、新しいログイン画面、外部アカウント連携先の追加をまとめた更新です。旗鯖以外のサーバーでもHataskeyを利用できるように整えました。あわせて花常の提供を終了します。

### Hatask

- **予定とToDoの大幅改良**: カレンダーを月・週・日・予定一覧で切り替え、予定を別の日や時刻へ動かしたり複製したりできるようにしました。ToDoは今日・これから・優先度・フォルダなどで整理し、複数の項目をまとめて変更できます
- **みんなのお花**: 育てた花を共有できるギャラリーを追加しました。花名・花言葉・育てた人を表示し、新しい順・古い順で眺められます。花の公開先は全体・フォロワー・自分のみから選べます
- **記録の保護**: 既存の予定・ToDoを引き継ぎ、読み込みに失敗したときに空の内容で保存しないようにしました。ほかの画面や端末からの変更とぶつかった場合も、記録を黙って上書きしないようにしています
- **きもちとごはんの記録**: 入力と過去の記録を見直し、既存の内容を保ちながら書きやすくしました。予定・ToDo・ごはんのテンプレートはひとつのアイコンから呼び出せます。きもちにはテンプレートを設けず、そのときの気持ちをそのまま残せます

### 新しい入口とサーバーごとの利用

- **ログイン画面の刷新**: Hataskeyの各機能を動く見本で紹介する画面へ統一しました。説明の文言、改行、アイコン、短い演出を整え、テーマの切り替えも滑らかにしました
- **サーバーの様子**: ログイン・登録ボタンの下に、サーバーの投稿、サーバー人数、アクティブ人数（7日平均・昨日）を表示します。スマートフォンでは投稿を最初に表示し、左右の矢印ボタンから切り替えられます
- **Hataskeyを別サーバーでご利用いただけるようになりました**: サーバーの名前・背景・紹介文・登録設定を使って入口を表示します。シンプルな入口も新しいデザインに揃え、以前のClassic／Simpleの選択設定は撤去しました。他言語のサーバー紹介文でもプラットフォーム名をHataskeyへ揃えました
- **登録方法の切り替え**: 申請受付のオン・オフを実際の登録方法に反映し、案内文も切り替えます。一般登録を開放している間は、管理画面の申請管理を「現在は登録を一般開放中です」と表示し、申請の承認・却下などを操作できないようにしました。既存の申請データは削除しません
- **参加申請の連絡先**: すでに使っているSNSなどの連絡先を、任意で記入できるようにしました。承認・拒否のどちらでも申請データから自動削除します。規約とルールは各サーバーの設定を使います
- **新しい申請のお知らせ**: 新しい参加申請が届くと、管理者だけに標準の通知欄でお知らせします。通知からコンパネの申請管理へ進めます。通知本文には応募者の名前や連絡先を含めません

### 連携と提供終了

- **BearBearを追加**: 外部アカウント連携の接続先にBearBear（xiapopisland.top）を追加しました。接続先のサーバーでは、そのサーバーの規約が適用されます
- **花常の提供終了**: ゲームの入口、機能紹介、関連する管理画面と専用APIを取り外しました。専用ソースと素材はリポジトリの保管用ディレクトリに残しますが、通常のビルド・配信には含めません。

### 毎日の使いやすさと安全性

- 廃止したポータルを保存済みのメニューからも取り除き、移行済みの印はアカウント・設定プロフィールごとに管理するようにしました。設定の読み込みに失敗した場合は変更を保存しません
- 設定画面の左メニューは、開く・閉じるを途中で切り替えても動きが途切れず、現在の幅から滑らかに切り替わるようにしました
- 申請管理の権限確認に加え、処理中に登録方法が切り替わった場合も申請の承認・却下を続行しないようにしました
- 古い却下済み申請の整理では、確認だけの操作でデータを変更せず、実行時は実際に変更できた件数を返すようにしました
- 任意の連絡先の削除と申請の承認・拒否を同じ処理で確定し、競合時も片方だけが処理されるようにしました。審査中の申請だけ管理者へ連絡先を返し、申請内容を診断ログへ残さないようにしています。通知送信の失敗で保存済み申請を取り消すことはありません
- 「今回の更新内容」を新しい7項目へ入れ替え、日本語・英語・簡体字中国語の案内を揃えました。内容別の見本は短く一度だけ動かし、動きを減らす設定でも案内を読めるようにしています

### 連合・保存データ・素材について

- この項に記載したログイン画面・外部アカウント連携先追加・申請受付の切り替え・花常の撤去では、ActivityPubの通信方式を変更しません
- 花常の提供終了に伴うデータベースの削除・破壊的な移行は行いません
- 任意の連絡先を保持する列と「審査完了後は連絡先を保持しない」制約を追加するデータベース更新を含みます。適用前にバックアップと検証用コピーでの確認が必要です。既存の申請情報を一括削除する更新ではありません
- 更新案内の見本は既存のTablerアイコンと画面部品で作成し、新しい外部素材や依存ライブラリは追加していません
