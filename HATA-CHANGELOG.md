...
hata-12.5.3以降の更新です。[Misskey 2026.9.0](https://github.com/misskey-dev/misskey/releases/tag/2026.9.0)のセキュリティ修正、メディアビューアーの改善、依存関係と翻訳の更新を取り込みました。Hataskey独自の画面構成、リアクションミュート、プライベートチャンネルの権限に合わせて調整しています。

### セキュリティと認証

- **公開範囲の確認**: リアクション一覧を取得する前にノートの閲覧権限を確認します。よく返信する相手の集計でも、集計元と返信先の公開範囲を確認し、Hataskeyのプライベートチャンネル権限を適用します
- **絵文字の一括インポート**: ZIPの取り込みを管理者だけに限定しました。展開するファイルのパス、重複、シンボリックリンク、暗号化、展開後のサイズなどを検査し、不正なアーカイブを拒否します
- **パスキーとセキュリティキー**: 登録・認証・パスキーログインで使うチャレンジの保存先を分離し、別の手順のチャレンジを流用できないようにしました。パスキーログインの識別子の形式も検査します
- **アクセストークンの失効**: 連携アプリは、使用中のトークン自身を失効できるようになりました。他のトークンや他人のトークンを失効できないよう、所有者と認証方法を確認します
- **メールとログ**: メール本文のHTMLを無害化し、件名や埋め込むURLなどをエスケープします。ログの正規化処理も見直しました

### メディアと画面表示

- **音声ビューアー**: 音声プレイヤーを画像・動画ビューアーへ統合しました。再生位置や音量の操作と音声ビジュアライザーを備え、Hataskeyの配色、アニメーション設定、端末の「動きを減らす」に合わせて表示します
- **画像の拡大とぼかし**: ピクセルアート向けの拡大表示を追加しました。メディア一覧でぼかしを解除してからビューアーを開くと、その状態を引き継ぎます。Blurhashが表示されない場合がある問題も修正しました
- **リアクション**: 「利用できるリアクションを先頭に表示」の判定と並び順を修正しました。Hataskey独自のリアクションミュートと、表示更新時の再取得制御は維持しています
- **通知トースト**: 左側へ置いた通知も指定した側から出入りするようにし、表示位置とアニメーションの挙動を修正しました
- **操作と応答**: スマートフォンで一部の文字を選択できない問題、管理画面のメール検索でエラーが二重に表示される問題、リストの人数が追加・削除後に更新されない問題を修正しました。設定検索とワードミュートの処理も改善しています
- **実験的設定の整理**: 「Enable condensed line」のスイッチを外しました。Hataskeyでは更新前から表示処理に使われていなかった設定で、保存済みプロファイルを読むための定義は残しています
- **翻訳**: ペルシャ語を追加し、各言語の翻訳を更新しました。Hataskey独自の文言は保持しています

### サーバーと互換性

- **リアクション取得の方式**: `notes/reactions`のGETと共有キャッシュを廃止しました。このエンドポイントを使う連携アプリはPOSTへ変更してください。Hataskey内の呼び出しは独自画面も含めて変更済みです
- **管理権限**: 絵文字ZIPの一括インポートは、ロールポリシーの有無にかかわらず管理者だけが実行できます。キューの閲覧には`read:admin:queue`を指定し、絵文字インポートには`write:admin:emoji`を指定しました
- **パスワード変更先**: `/.well-known/change-password`からセキュリティ設定へ移動できるようにしました
- **サーバー処理**: すでにミュートしたスレッドを再度ミュートするとエラーになる問題と、一部のWebSocketチャンネルの後片付けを修正しました。キュー処理を更新後のライブラリへ合わせ、センシティブメディア判定サービスの名称も整理しています
- **依存関係とSDK**: pnpm 11.25.0、Vue 3.5.42、Vite 8.2.2などへ更新し、HataskeyのAPI定義からcherrypick-jsを再生成しました。本体とSDKの`basedMisskeyVersion`を2026.9.0へ揃えています
- Hataskeyで提供を終了したノート翻訳機能は復活させていません。上流のCW翻訳の変更は対象外です

### 移行・連合・ライセンス

- 新しいデータベースマイグレーションとNode.jsのメジャー更新は含みません。既存の予定・ToDo・テーマ設定の保存先は変更していません
- **ActivityPubへの影響**: 古いアカウントのRSA秘密鍵を読み込む際、PKCS#1形式をPKCS#8形式へ変換して保存し、連合できない場合がある問題に対応しました。変換後も同じ鍵として署名できることと、保存に失敗した場合の処理を単体テストで確認しています。実際の連合先とのHTTP署名の疎通確認は別途必要です
- 絵文字ZIP処理に`@zip.js/zip.js`（BSD-3-Clause）を追加しました。Hataskey全体の配布条件はAGPL-3.0-onlyを維持します
- アプリ内の更新案内を日本語・英語・簡体字中国語で更新しました。既存の4タブ構成でhata-12.5.4・hata-12.5.3・hata-12.5.2・hata-12.5を表示し、hata-12.5.1以前の履歴もこのチェンジログに残しています
