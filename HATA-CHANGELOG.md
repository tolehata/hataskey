...
Hataskのカードメーカーとお花の成長、HataSideStudio、HatasabaUIデッキ、HataSNSCordUI、情報ページを改善する機能更新です。
...
### Hatask・HatasabaUIの修正

- Hataskのお花をページの表示状態に依存せず実経過時間で育て、画面を閉じている間やバックグラウンドの時間も次回起動・復帰時に反映するようにしました。花のレジストリをまだ持たない利用者でも初回の花が保存され、育ち始めます
- HatasabaUIデッキでツールバーの時計を表示したとき、秒表示の更新に合わせて通知カラムやほかのカラムが再描画されて点滅する問題を修正しました。時計だけを独立して更新し、カラムへ渡す設定値も安定した参照で保持します
...
- **UI内で完結する画面遷移**: 設定、コントロールパネル、旗鯖独自ツール、ゲーム、プロフィールなどを開いてもHatasabaUIへ戻らず、左右のペインを維持したまま中央ペインで表示します
- **右サブペインの作業タブ**: 投稿詳細、プロフィール、検索、通知、お知らせ、ドライブ、チャット、各タイムライン、リスト、アンテナ、チャンネル、ウィジェットを追加・削除・並び替えできます。HatasabaUIのウィジェット構成も取り込めます
...
- **旗鯖独自機能の英語・中国語（簡体字）対応**: Hatady、Hatask、HataFeed、HataSNSCordUI、HataSideStudio、HatasabaUI、旗鯖独自設定、外部アカウント連携・通知、登録申請、マスコット、お絵描きツール、独自ゲームなどの表示を、本体の共通言語設定へ統合しました。Hatadyの独自言語選択は廃止し、保存済みの記録や設定を変えずにHataskey本体の表示言語へ従います。利用者が入力した投稿・記録・名前は翻訳しません。花常と地震・津波情報は対象外です
...
- Botフラグ付きアカウントからの通知を、通常の通知メニュー、通知ウィジェット、HatasabaUIデッキ、HataSNSCordUIのサブペインで非表示にできるようにしました
...
- **HataSideStudio**: 実際のサイドメニューを見ながら、ボタン・グループ・全ウィジェットの配置、形、配色、サイズ、表示内容を端末ごとに編集できるスタジオツールを追加しました。拡大時と縮小時の個別構成、複数プロファイル、ドラッグと簡易タイムライン、元に戻す・やり直し、モバイル表示、設定の書き出し・読み込みに対応し、Hataskとサーバーメニューからも起動できます。HatasabaUIデッキ上でも作成・確認・チュートリアルの小窓が手前に表示されます
- **HatasabaUI 2**: ノート、カラム、上部・下部のメニューをすりガラス調で統一する新しい表示を追加しました。透過の強さなどは独立した設定画面で確かめながら調整できます
- **HatasabaUIデッキの機能追加**: クリップ・お気に入りのカラム、カラムごとの再読み込み、チャンネル投稿ボタン、ウィジェット編集への導線を追加しました
- **Bot投稿の非表示**: Botの投稿をタイムラインから隠し、必要なBotだけを例外として表示できる設定を追加しました
- **Hataskのお花ウィジェット**: 育成中の花を円形の進み具合で示し、咲いた花の一覧と総数を通常表示とHatasabaUIデッキの両方で確認できるようにしました
...
- **左右スワイプの停止設定**: HatasabaUIのタブを左右スワイプで切り替えないよう、端末ごとに設定できるようにしました。初期状態は従来どおり有効です
- **チャンネルとプロフィールのタブを整理**: 選択中の項目が分かりやすい丸みのある表示へ統一しました
- **HatasabaUI 2の表示を安定化**: チャンネル色の縦線、日付の区切り、Bot非表示後の空白、従来デッキとの投稿ボタン重複などを修正しました
- **設定画面を見比べやすく**: HatasabaUIとサイドバーの編集画面を、背後の表示を操作しながら調整できる独立ウィンドウへ変更しました
- **UI選択画面の見切れを修正**: 画面より内容が高いときに下までスクロールできるようにし、スマートフォンの安全領域にも対応しました
- **「みつける」と「もっと！」の戻り道を修正**: HatasabaUIで戻るボタンが出ない問題と、マスコットがサイドバーにも「もっと！」にも現れない問題を修正しました
...
- **Hatadyに戻るボタンを追加**: 通常表示のパソコンとスマートフォンの左上から直前の画面へ戻れるようにしました。HatasabaUIデッキではデッキ側の操作と重複しないよう非表示にします
...
- **旗鯖機能解説を全面整理**: HatasabaUIとHataFeedの重複説明をまとめ、専門用語を利用者向けの表現へ改め、プライベートチャンネルと投稿範囲別の色分けを追記しました
