...
- 通知フィルタの修正、HataFeed通知の既読更新、HataSNSCordUIの1時間単位の操作上限は、自サーバー内の通知取得・既読状態と認証済みAPI操作だけを対象とし、ActivityPubの送受信・署名・連合キューには変更を加えていません
- 言語対応は画面と通知の表示時変換に限定し、投稿本文、利用者が入力した内容、ActivityPubの送受信・署名・連合キューには変更を加えていません
- Bot通知の除外とBot・人間別のグループ表示は、通知の生成や配送を変えず、取得時と端末上の表示だけに適用します。ActivityPubの送受信、HTTP Signatures、連合キューには影響しません
