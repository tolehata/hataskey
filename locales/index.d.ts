...
export interface Locale extends ILocale {
    ...
    "_hata": {
        ...
        "_whatsNew": {
            ...
            "_content": {
                /**
                 * hata-12.6.2 — ランキングを追加し、通知設定と参加申請を改善しました
                 */
                ...
                "rankingTitle": string;
                /**
                 * お花の数・宴の成功数・宴の阻止数・ログイン日数を、今月・今週・今日で比べられるランキングをHataskに追加しました。初期状態は参加です。ランキング画面でいつでも不参加に切り替えられます。プロフィールで非表示の花数・宴の記録や、非公開の花ギャラリーは表示しません。
                 */
                "rankingText": string;
                /**
                 * 選んだ通知フィルターを保存
                ...
                /**
                 * 通知画面のBOT除外設定が保存されるようになりました。ウィジェットやデッキの通知フィルターも、別のタブや端末との同期で古い設定に戻る問題を修正しました。旧デッキからの移行でもフィルターを引き継ぎます。
                 */
                ...
                /**
                 * 参加申請で、サーバー管理者と関係がある方はチェックを付け、理由の代わりにSNSなどの連絡先を入力できるようになりました。申告した場合は連絡先が必須です。申請はこれまでどおり管理者が審査します。
                 */
                ...
                /**
                 * ミュートしたモデレーターのリアクションも非表示になります。サーバー管理者へのミュート・ブロックは禁止のままです。入口画面に不要な横スクロールが出る問題も修正しました。
                 */
                ...
                "hataskLink": string;
                /**
                 * hata-12.6.2の主な変更をご紹介しました。詳しい変更点と過去の履歴はリリースノートをご覧ください。
                 */
                "footerText": string;
                /**
                 * リリースノートを見る
