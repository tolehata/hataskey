...
export interface Locale extends ILocale {
    ...
    "_hata": {
        ...
        "_whatsNew": {
            ...
            "_content": {
                /**
                 * hata-12.6.1 — お花を眺めやすく、通知と表示の不具合を修正しました
                 */
                "latestHeadline": string;
                /**
                 * お花を滑らかに眺め、一覧から選べるように
                 */
                "flowerTitle": string;
                /**
                 * みんなのお花とフラワーギャラリーの自動スクロールが止まる・がたつく問題と、タップした花の詳細がすぐ閉じる問題を修正しました。花名は2行まで表示し、自分のギャラリーではアカウント名の繰り返しを省きます。一時停止ボタンの左に一覧ボタンを追加し、並び替え・ページ切り替えは一覧の中へ移しました。
                 */
                "flowerText": string;
                /**
                 * 暁のアプリ一覧を、ダークモードでも見やすく
                 */
                "hataskHomeTitle": string;
                /**
                 * 暁のHatask App・Hataskey Appで、機能カードが灰色にくすむ問題を修正しました。カードの背景を暁の配色に揃えています。保存したテーマの選択や、予定・ToDoの内容はそのまま使えます。
                 */
                ...
                /**
                 * Hataskey UIで、タッチ操作やアプリへ戻ったあとに通知が閉じず残る問題を修正しました。ほかのポップアップを開いていても表示時間は進みます。スマートフォンでは、アカウントアイコンを上部ナビ内の左端へ収め、新着ノートの案内をタブの下へ揃えました。
                 */
                "notificationsText": string;
                /**
                 * iPhoneのホーム画面から開いた花壇の表示を修正
                 */
                "dailyPolishTitle": string;
                /**
                 * iOS Safariからホーム画面へ追加したアプリで、花壇の葉や茎は見えるのに花が表示されない問題に対応しました。選択中の絵文字スタイルを引き続き使い、花の種類・配置や育てたお花のデータは変わりません。
                 */
                ...
                "hataskLink": string;
                /**
                 * hata-12.6.1の主な変更をご紹介しました。詳しい変更点と過去の履歴はリリースノートをご覧ください。
                 */
                "footerText": string;
                /**
                 * リリースノートを見る
