...
try {
	...
	await Promise.all([
		...
		runChildProcess('pnpm', ['--filter', 'backend...', 'build'], {
			cwd: _dirname + '/../',
			stdout: process.stdout,
			stderr: process.stderr,
		}),
	...
	startChildProcess('pnpm', ['--filter', 'frontend-shared', 'watch', '--no-clean'], {
		cwd: _dirname + '/../',
		stdout: process.stdout,
		stderr: process.stderr,
	});
	...
	startChildProcess('pnpm', ['--filter', 'cherrypick-js', 'watch', '--no-clean'], {
		cwd: _dirname + '/../',
		stdout: process.stdout,
		stderr: process.stderr,
	});
