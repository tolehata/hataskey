...
// 旗鯖fork: 本家 Misskey 2026.6.0 から取り込み (OAuth2 認可画面 oauth.tsx 依存)。
// vanilla UI 全体は旗鯖 pug (base.pug) を引き続き利用するため、この Layout は
// OAuth2 認可画面など本家由来の JSX ページ専用。
		...
		<>
			...
			<html>
				<head>
					...
					<link rel="search" type="application/opensearchdescription+xml" title={props.title || 'Hataskey'} href={`${props.config.url}/opensearch.xml`} />
					...
					{props.titleSlot ?? <title safe>{props.title || 'Hataskey'}</title>}
					...
					{props.ogSlot ?? (
						<>
							<meta property="og:title" content={props.title || 'Hataskey'} />
