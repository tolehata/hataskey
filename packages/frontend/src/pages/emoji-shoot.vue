	...
	<div class="_spacer" style="--MI_SPACER-w: 600px;">
		<div :class="$style.root">
			<div class="_gaps">
				...
				<div class="_panel" :class="$style.menuCard">
					<div class="_gaps" style="padding: 20px;">
						<MkButton primary gradate large rounded inline @click="startGame('normal')">
							<i class="ti ti-player-play"></i> {{ copy.normalMode }}
						</MkButton>
						<MkButton large rounded inline @click="startGame('debuff')" style="background: linear-gradient(135deg, #ff6b6b, #e74040); color: #fff; border: none;">
							<i class="ti ti-skull"></i> {{ copy.debuffMode }}
						</MkButton>
						<div style="font-size:.8rem;opacity:.5;text-align:center;">{{ copy.debuffDescription }}</div>
					</div>
				</div>
						...
						<div v-else :class="$style.rankList">
							<div v-for="(r, i) in ranking" :key="r.id" :class="$style.rankItem">
								...
								<div :class="$style.rankInfo">
									...
									<div :class="$style.rankMeta">{{ copyx.waveKills({ wave: String(r.wave), kills: String(r.kills) }) }}</div>
								</div>
				...
				<div class="_panel" :class="$style.menuCard">
					<div class="_gaps_s" style="padding: 16px;">
						<b>💀 {{ copy.debuffRanking }}</b>
						<div v-if="debuffRankingLoading" style="text-align:center;padding:16px;"><MkLoading/></div>
						<div v-else-if="debuffRanking.length === 0" style="text-align:center;padding:16px;opacity:.5;">{{ common.noRecords }}</div>
						<div v-else :class="$style.rankList">
...
const copy = i18n.ts._hata._games._shoot._home;
const copyx = i18n.tsx._hata._games._shoot._home;
...
definePage(() => ({ title: copy.title, icon: 'ti ti-rocket' }));
</script>
