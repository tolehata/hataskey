...
const categories = [
	...
	{
		id: 'shrimpia', iconClass: 'ti ti-fish', label: 'シュリンピア連携',
		docs: [
			{ iconClass: 'ti ti-link', title: 'シュリンピア連携とは', body: `シュリンピア (mk.shrimpia.network) と連携して、旗池2丁目から直接シュリンピアのタイムラインを見たり、投稿・リアクション・リプライ・リノートができる機能です。<br><br>MiAuth認証で安全にアカウント連携を行います。`, tips: ['連携前にシュリンピアの利用規約をご確認ください', 'シュリンピアでの行為にはシュリンピアの規約が適用されます'], link: '/settings/external-account', linkLabel: '連携設定を開く' },
			{ iconClass: 'ti ti-device-tv', title: '外部タイムライン (OHTL / OLTL)', body: `連携後、2種類の外部タイムラインが利用可能になります。<br><br><b>OHTL（外部ホームTL）</b>: シュリンピアでフォローしているユーザーのノートを表示します。<br><br><b>OLTL（外部ローカルTL）</b>: シュリンピアのローカルタイムラインを表示します。<br><br>タイムラインタブに追加され、通常のタイムラインと同じ感覚で切り替えて使えます。`, tips: ['外部TLからそのまま投稿・リアクション・リプライ・リノートが可能', 'シンプルTLでは＋マークのタブでアクセスできます'], link: '/settings/external-account' },
	...
	},
	{
		...
		docs: [
			...
			{ iconClass: 'ti ti-layout-list', title: 'シンプルTL（モダンシンプルUI）', body: `旗鯖オリジナルのタイムライン特化型UIです。<br><br>画面上部のタブでHTL（ホーム）・LTL（ローカル）・GTL（グローバル）をワンタップで切り替えられます。シュリンピア連携時は＋H（外部ホーム）・＋L（外部ローカル）タブも追加されます。<br><br><b>特徴:</b><ul><li>スワイプでタイムライン切り替え</li><li>画面下部のピル型ナビバーから主要機能へアクセス</li><li>スクロール時にナビバーが自動で隠れて画面を広く使えます</li><li>KeepAliveでタブ切り替え時の再読み込みを防止</li></ul>`, tips: ['ドロワーメニューからサイドバーを開けます', '外部TLの通知や投稿もシンプルTL内のピル型ボタンで操作可能'] },
