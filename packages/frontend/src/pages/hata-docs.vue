...
const categories = [
	{
		...
		docs: [
			...
			{ iconClass: 'ti ti-bowl', title: 'ごはん記録', body: `その日の食事をゆるやかに振り返るためのセルフケア機能です。きもち記録と並ぶタブとして用意されています。<br><br>記録は「いつのごはん（朝・昼・夜・間食）」「食べられたか（食べれた / 少しだけ / 食べれなかった）」を選ぶだけのシンプルな形式です。必要なら理由（任意・複数選択可）やひとことメモも添えられます。<br><br><b>このツールはカロリーや量を計算したり、点数をつけたりはしません。</b>「食べられた／食べられなかった」はどれも等しく中立に扱われ、達成率や連続記録のようなプレッシャーになる表示も行いません。あくまで「今日はこうだったな」と穏やかに振り返るためのものです。`, tips: ['3つの状態はすべて等価に扱われ、できなかった日を責める表示はしません', '初回表示時に用途についての説明が出ます（右上の「!」からいつでも再確認できます）', '本ツールは医療目的のものではありません。体調の不安が続くときは医療機関にご相談ください'] },
	...
	},
	{
		...
		docs: [
			...
			{ iconClass: 'ti ti-layout-list', title: 'Hatasaba UI', body: `旗鯖オリジナルのタイムライン特化型UIです。<br><br>画面上部のタブでHTL（ホーム）・LTL（ローカル）・GTL（グローバル）をワンタップで切り替えられます。外部アカウント連携時は＋H（外部ホーム）・＋L（外部ローカル）タブも追加されます。<br><br><b>特徴:</b><ul><li>スワイプでタイムライン切り替え</li><li>画面下部のピル型ナビバーから主要機能へアクセス</li><li>スクロール時にナビバーが自動で隠れて画面を広く使えます</li><li>KeepAliveでタブ切り替え時の再読み込みを防止</li></ul><br><b>表示モードの切り替え（デスクトップ）:</b><ul><li><b>通常表示 / デッキ表示</b>をワンタップで切り替えられます</li><li>ナビゲーションを<b>上部メニュー / 左サイドメニュー</b>から選べます</li><li>左サイドメニューは手動で縮小・拡大でき、続きがある時は上下のフェードで示されます</li></ul>`, tips: ['ドロワーメニューからサイドバーを開けます', '外部TLの通知や投稿もHatasaba UI内のピル型ボタンで操作可能', 'デスクトップでは通常表示とデッキ表示を切り替えられます', 'ナビは上部メニューと左サイドメニューを選べます'] },
			{ iconClass: 'ti ti-columns', title: 'HatasabaUI デッキ表示', body: `Hatasaba UIのデスクトップで、複数のカラムを並べて表示するデッキ表示が使えます。<br><br>デッキ表示中はノートの間隔が自動的に詰まり、一度に多くの情報を見渡せます。デッキ用のお知らせ吹き出しは個別に無効化できます。<br><br><b>カラムに追加できるタブ (hata-11.8〜):</b><ul><li>ホーム / ローカル / グローバル / ハイブリッド / メンション / ダイレクト / リスト / アンテナ / チャンネル / トレンド / 通知</li><li><b>クリップ / お気に入り</b> — カラム化に対応</li><li>外部タイムライン (OHTL / OLTL)</li></ul><br><b>カラム操作:</b><ul><li>各カラムの三点メニュー/ヘッダのリロードボタンから <b>そのカラムだけを再取得</b> できます</li><li>チャンネルカラムでは、フローティングの<b>チャンネル投稿ボタン</b>からその場で投稿できます (HatasabaUI 2 デッキ限定)</li></ul>`, tips: ['デッキ表示はデスクトップ向けです', 'ノート間隔は自動で詰まります', 'クリップ/お気に入りはストリーミング対象外のため、カラムのリロードボタンで手動更新できます'] },
			{ iconClass: 'ti ti-sparkles', title: 'HatasabaUI 2 (ベータ)', body: `HatasabaUI のグラスモーフィズム基調刷新版です (hata-11.8〜、ベータ)。<br><br>ノートカードや上部/下部ナビバーをテーマ accent 由来の<b>ガラス面</b> + backdrop-filter でまとめて描画し、背景のヘッダー画像/デッキ背景ぼかしを透かして「浮かせる」ような見た目にします。<br><br><b>対応範囲:</b><ul><li>ホーム / ローカル / グローバル / チャンネル / リスト / アンテナ / メンション / ダイレクト / 外部 (OHTL/OLTL) / 通知 / クリップ / お気に入り / トレンドの<b>すべてのノート含有カラム</b></li><li>HatasabaUI 2 デッキの<b>全カラム</b>で同じ透過が反映されます</li></ul><br><b>設定は独立ウィンドウ:</b><br>旗鯖設定 → HatasabaUI 2 タブ内の「設定を開く」から独立ウィンドウが開きます。ドラッグして移動でき、裏のタイムラインを見ながら調整できます。「保存」で永続化 + 自動リロード、「初期値に戻す」で全値をリセット。<br><br><b>調整できる項目:</b><ul><li>ON/OFF (端末ローカル)</li><li>透過率スライダー 0-100% (マルチデバイス同期、default 55%)</li><li>吹き出しモード時の追加ガラス</li><li>プロフィールバナー無し時の背景</li><li>通常/デッキそれぞれの吹き出しの有無</li></ul><br>ON/OFF は端末ローカル (miLocalStorage) なので、モバイルで負荷が気になる時は PC でだけ有効化する運用ができます。`, tips: ['透過率スライダーはウィンドウを開いたままリアルタイムで試せます', '設定を保存すると自動でリロードされます', 'ダーク/ライト切替やテーマ変更にもそのまま追従します'], link: '/settings/hata-custom', linkLabel: 'HatasabaUI 2 の設定を開く' },
			...
			{ iconClass: 'ti ti-robot', title: 'Bot 投稿の非表示 (許可アカウント指定可)', body: `<code>isBot: true</code> なユーザーの投稿をタイムラインから非表示にできます (hata-11.8〜)。<br><br>例外的に表示したい bot は「許可アカウント」から個別に追加できます。ユーザー検索ダイアログから追加/削除可能です。<br><br><b>判定ロジック:</b><ul><li>Bot 自身のオリジナル投稿 → <b>非表示</b></li><li>通常ユーザーが Bot の投稿をリノート → <b>非表示</b> (リノート元が bot と判定)</li><li>Bot が通常ユーザーの投稿をリノート → <b>表示</b> (リノート先が通常ユーザーと判定)</li><li>許可アカウントに追加した Bot → <b>常に表示</b></li></ul><br>設定はマルチデバイス同期 (prefer 保存) なので、1 端末で設定すれば他端末にも自動反映されます。`, tips: ['「旗鯖独自設定 → 旗鯖全体」タブから設定できます', '許可アカウントは複数登録可能で、いつでも削除できます', '通知・引用元・埋め込みノートには適用されません (経路が異なるため)'], link: '/settings/hata-custom', linkLabel: '設定を開く' },
			{ iconClass: 'ti ti-cloud-rain', title: '天気エフェクト', body: `タイムラインのノート本文に天気や挨拶の言葉が含まれていると、画面の背景にそっと演出が表示される遊び要素です。<br><br><b>対応する演出（全6種）:</b><ul><li><b>雨</b>: 「雨」「小雨」「夕立」など</li><li><b>大雨</b>: 「土砂降り」「豪雨」「大雨」など</li><li><b>雪</b>: 「雪」「吹雪」など</li><li><b>日差し</b>: 「晴れ」「快晴」、朝の挨拶「おはよう」など</li><li><b>強風</b>: 「強風」「突風」「木枯らし」など</li><li><b>流れ星</b>: 「いい夜」「おやすみ」「流れ星」など就寝の挨拶</li></ul><br><b>UIと連動する仕掛け:</b><ul><li>雨・大雨のときは、ノートボタンや投稿フォーム・ナビバーの上に水が当たって伝い落ちます</li><li>マウスを動かすと周りの雨を弾けます</li><li>日差しのときは、マウスカーソルに光源と反対向きの影がつきます</li><li>強風のときは、流れる葉っぱがサイドメニューなどの縁に引っかかってくるくる回ります</li></ul><br>「おはよう」「おやすみ」などの挨拶で出た演出は約10秒で自然に消えます。それ以外の天気は設定で「長め（ノートがある間ずっと）」「短め（約10秒）」を選べます。<br><br><b style="color: var(--MI_THEME-warn);">光過敏症への配慮:</b> 強い明滅・点滅・閃光は一切行いません。雷のような演出も安全のため実装していません。OSの「視差効果を減らす」設定にも追従します。光や動きに違和感を覚えた場合は、設定からいつでもOFFにできます。`, tips: ['デフォルトはOFFです。旗鯖設定から有効にできます', '演出の長さは「長め／短め」から選べます', '挨拶（おはよう／おやすみ等）の演出は常に約10秒で消えます', '少しでも体調に違和感を覚えたらすぐにOFFにしてください'], link: '/settings/hata-custom', linkLabel: '旗鯖設定を開く' },
		],
	},
	{
	...
	},
	{
		...
		docs: [
			...
			{ iconClass: 'ti ti-user-check', title: '登録申請インジケーター（管理者向け）', body: `承認制登録の運用時、未処理の登録申請があると、コントロールパネルにインジケーターと警告が表示されます。<br><br>申請の見落としを防ぎ、スムーズに承認作業へ移れます。`, tips: ['この機能は管理者・モデレーター向けです'] },
			{ iconClass: 'ti ti-list-details', title: '全チャンネル一覧タブ（管理者向け）', body: `チャンネルページに、管理者・モデレーターだけに表示される「<b>すべてのチャンネル</b>」タブが追加されます (hata-11.8〜)。<br><br>非表示 (センシティブ) や高度な絞り込みが必要なチャンネルも含め、モデレーション対象を一覧で把握できるようにします。バックエンドには専用エンドポイント <code>admin/channels/list</code> (<code>requireModerator</code> + <code>read:admin:channels</code> スコープ) が追加されており、通常ユーザーからは呼び出せません。<br><br>あわせて、チャンネル編集画面のサブ管理者一覧の表示も整理され、権限付与状態を把握しやすくなっています。`, tips: ['この機能は管理者・モデレーター向けです', '通常ユーザーのチャンネル一覧には表示されません'], link: '/channels', linkLabel: 'チャンネル一覧を開く' },
		],
	},
	{
		id: 'mascot', iconClass: 'ti ti-mood-happy', label: 'マスコット',
		docs: [
			{ iconClass: 'ti ti-mood-happy', title: 'マスコット機能とは', body: `画面にあなただけのキャラクター（マスコット）を表示できる旗鯖独自の機能です。好きな画像を立ち絵として登録し、表情やセリフを設定して、画面の好きな位置に置いておけます。<br><br>キャラクターは複数登録でき、いつでも切り替えられます。設定はマスコット専用ページから行います。`, tips: ['画像はドライブから選択できます', 'キャラクターは複数作成して切り替え可能', '利用にはロール（権限）が必要な場合があります'], link: '/mascot', linkLabel: 'マスコット設定を開く' },
			{ iconClass: 'ti ti-drag-drop', title: 'フローティング表示', body: `マスコットを画面上にフローティング（浮遊）表示できます。<br><br><b>主な操作・設定:</b><ul><li>ドラッグで好きな位置へ移動（位置は記憶されます）</li><li>最小化／元に戻す</li><li>左右反転</li><li>透過度の調整</li><li>ぼかしの調整</li><li>簡易設定パネルからその場で各種調整</li></ul>表示のON/OFFはいつでも切り替えられます。`, tips: ['ドラッグした位置は次回も保持されます', '簡易設定パネルから素早く調整できます'] },
			{ iconClass: 'ti ti-mood-smile', title: '表情とセリフ', body: `キャラクターに複数の表情と、それぞれに対応するセリフ（文言）を登録できます。<br><br>表情には動き（モーション）を付けられます。<ul><li><b>モーション</b>: なし／バウンス／シェイク／スウェイ／スピン</li><li>モーションの強さも調整可能</li></ul>セリフはフキダシで表示され、フキダシの位置・大きさ・向き・文字色も細かく設定できます。`, tips: ['表情ごとにモーションと強さを設定できます', 'フキダシの位置や色も調整可能'] },
			{ iconClass: 'ti ti-refresh', title: '表情・文言の自動切替', body: `表示中のマスコットの表情やセリフを、一定間隔で自動的に切り替えられます。<br><br>切替間隔はプリセット（または手動入力）で<b>5秒〜30分</b>の範囲で設定できます。設定は専用ページとフローティング表示で共通です。`, tips: ['間隔はプリセットから選ぶか手動で入力できます'] },
			{ iconClass: 'ti ti-bell', title: '通知・誕生日の演出', body: `通知が届いたときや誕生日に、専用の表情とセリフを表示できます。<br><br><b>通知用表情</b>は2種類まで登録でき、通知時にランダムで切り替わります。Hataskの通知や外部アカウントの通知とも連携します。<br><br><b>誕生日</b>には、あなたの誕生日やキャラクター自身の誕生日に合わせた特別な表情を表示できます。`, tips: ['通知用表情は2種登録でき通知時にランダム表示', 'Hatask・外部通知とも連携します', 'キャラクター自身の誕生日も設定可能'] },
			{ iconClass: 'ti ti-home', title: 'Hataskホームのマスコットカード', body: `Hataskのホーム画面にマスコットをカードとして表示できます。<br><br>フローティング表示と連動しており、二重に表示されないように自動で調整されます。`, link: '/hatask' },
			{ iconClass: 'ti ti-file-import', title: 'インポート / エクスポート', body: `マスコットの設定を <b>.hmtk</b> 形式のファイルとして書き出し（エクスポート）・読み込み（インポート）できます。<br><br>キャラクターの設定をバックアップしたり、他の端末へ移したりするのに使えます。`, tips: ['設定のバックアップや端末間の移行に便利です'], link: '/mascot' },
			{ iconClass: 'ti ti-alert-triangle', title: '設定ミスのガード', body: `画像が未指定だったり、誕生日の月日が未入力など、設定に不備がある場合は保存ボタンがグレーアウトし、理由が表示されます。<br><br>設定が正しく完了してから保存できるようになっています。` },
		],
	},
	{
		id: 'feedback', iconClass: 'ti ti-message-report', label: 'HataFeed（フィードバック）',
		docs: [
			{ iconClass: 'ti ti-message-report', title: 'HataFeed とは', body: `旗鯖への不具合報告・要望・改善提案などをまとめて受け付ける、旗鯖独自のフィードバックセンターです。<br><br><b>主な機能:</b><ul><li><b>イシュー（投稿）</b>: 不具合・未解決・機能要望・取入要望・その他などカテゴリ別に投稿できます</li><li><b>会話</b>: 各イシューにコメントを付けてやり取りでき、リアクション・特定コメントへの返信（スレッド）・重要/?マークが使えます</li><li><b>ステータス・解決</b>: 受付終了・解決済みなどの状態管理。クローズされたイシューは「みんなの動き」に明記されます</li><li><b>プロジェクト</b>: 旗鯖以外のプロジェクトのフィードバックも、ジャンルやリポジトリURL付きで管理できます</li><li><b>絵文字申請・通知</b>: 絵文字のリクエストや、自分のイシューへの反応をベル通知で受け取れます（1ページ5件・ページ送り）</li></ul>`, tips: ['利用にはロール（権限）が必要な場合があります', 'イシュー番号 #N はリンクになっていてタップで該当イシューに飛べます', 'Hataskの「旗鯖独自アプリ」や「もっと！」メニューからも開けます'], link: '/hatafeed', linkLabel: 'HataFeed を開く' },
			{ iconClass: 'ti ti-shield', title: 'セキュリティ対応イシュー', body: `セキュリティに関わるイシューは、スタッフ（管理者・モデレーター）のみが閲覧・作成できる内部限定カテゴリとして扱われます。<br><br>一般ユーザーの一覧や作成画面には表示されず、安全に取り扱えるようになっています。` },
		],
	},
	{
		id: 'bousai', iconClass: 'ti ti-activity', label: '防災（地震・津波）',
		docs: [
			{ iconClass: 'ti ti-activity', title: '地震・津波情報ビューア', body: `気象庁が発表した<b>発表済みの地震情報・津波情報</b>を、リアルタイムに表示する旗鯖独自の防災機能です（データ提供: 気象庁 / P2P地震情報）。<br><br><b>主な機能:</b><ul><li><b>日本地図</b>: 震源に最大震度を表示。マウス/タッチでパン・ズームでき、地点をタップすると<b>市区町村レベル</b>で震度を色分け表示します（出典: 国土数値情報）</li><li><b>一覧・電光掲示板</b>: 同じ地震の第○報は自動でまとめ、上部に最新地震の各地の震度を流れる電光掲示板で表示します</li><li><b>津波情報</b>: 発表中の津波情報を段階（大津波警報/警報/注意報）で表示。解除されると自動で消えます</li><li><b>付近の地震</b>: お住いの都道府県で観測された地震をまとめて表示します</li></ul><br><b style="color: var(--MI_THEME-warn);">緊急地震速報（EEW）は扱いません。</b>あくまで発表済みの情報の表示・通知に限定しています。`, tips: ['Hatasaba UI のデッキにカラムとして追加したり、ウィジェットとして配置できます（都道府県フィルター付き）', '地図の左上に、サーバーとのリアルタイム接続状況が常時表示されます'], link: '/earthquake', linkLabel: '地震・津波情報を開く' },
			{ iconClass: 'ti ti-bell', title: '地震・津波の通知', body: `地震・津波の<b>プッシュ通知</b>を受け取れます（アプリを閉じていても届きます）。<br><br><b>2つのモードから選べます:</b><ul><li><b>一定の震度以上で通知</b>: 選んだ震度（震度3〜7）以上の地震が起きたら通知</li><li><b>お住いの都道府県で揺れたら通知</b>: 自分の地域に震度が出たときだけ通知</li></ul><br><b style="color: var(--MI_THEME-warn);">プライバシーについて:</b> お住いの都道府県は通常<b>この端末にのみ保存</b>され、サーバーには送られません。「居住地のみ通知」を有効にしたときだけ、通知判定のために<b>都道府県名のみ</b>がこのサーバーに保存されます（市区町村やGPS等は一切送りません）。モードを変えたり通知をOFFにすると、サーバーから削除されます。`, tips: ['通知の設定は地震ページの歯車ボタンから行えます', '都道府県が未設定の場合「居住地のみ通知」は選べません'], link: '/earthquake' },
		],
	},
	{
		id: 'beta', iconClass: 'ti ti-flask', label: 'ベータ機能',
		docs: [
			{ iconClass: 'ti ti-flask', title: 'ベータ機能を試す', body: `HataFeed には、開発中の<b>ベータ機能</b>を試せる場所があります。正しく動くとは限らないため、不具合があればイシューでお知らせください。<br><br><b>現在のベータ機能:</b><ul><li><b>C/C++ プレイグラウンド</b>: ブラウザの中だけで C / C++ のコードを書いて実行できる遊び場です（コードはサーバーに送られません）</li><li><b>ミュートユーザーのリアクション非表示</b>: ミュートした人が付けたリアクションを、ノート上に表示しないようにします（この端末にだけ保存され、ほかの端末には引き継がれません）</li></ul>`, tips: ['HataFeed の「ベータ機能を試す」から利用できます', 'ベータ機能の数は HataFeed のボタンにバッジで表示されます'], link: '/hatafeed', linkLabel: 'HataFeed を開く' },
		],
	},
];
