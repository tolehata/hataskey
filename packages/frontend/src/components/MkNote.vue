...
>
    ...
    <MkNoteSub v-else-if="appearNote.replyId && !renoteCollapsed && !replyCollapsed && notification && prefer.s.showReplyInNotification" :note="appearNote?.reply ?? null" :class="$style.replyTo"/>
...
import { checkMuted as isEmojiMuted, makeEmojiMuteKey, extractCustomEmojiName, extractCustomEmojiHost } from '@/utility/emoji-mute.js';
import { filterHiddenReactions, getHiddenReactions, unhideReaction, hiddenReactionsVersion } from '@/utility/hidden-reactions.js';
...
// ミュートされた絵文字をリアクション表示からフィルタリング
const displayedReactions = computed(() => {
	// hiddenReactionsVersionを参照してリアクティビティを確保
...
const expandOnNoteClick = computed(() => prefer.s.expandOnNoteClick);
...
function noteDblClick(ev: MouseEvent) {
    if (!expandOnNoteClick.value || window.getSelection()?.toString() !== '' || prefer.s.expandOnNoteClickBehavior === 'click') ev.stopPropagation();
    else router.pushByPath(notePage(appearNote));
}
...
function showMenu(): void {
    ...
    // このノートに非表示リアクションがあれば復元メニューを追加
    const hiddenForThisNote = getHiddenReactions().filter(h => h.noteId === appearNote.id);
    if (hiddenForThisNote.length > 0) {
        menu.push({ type: 'divider' });
        menu.push({
            text: `非表示リアクションを復元 (${hiddenForThisNote.length})`,
            icon: 'ti ti-eye',
            action: async () => {
                const { canceled } = await os.confirm({
                    type: 'question',
                    text: `${hiddenForThisNote.length}件の非表示リアクションを復元しますか？`,
                });
                if (canceled) return;
                for (const h of hiddenForThisNote) {
                    unhideReaction(h.noteId, h.reaction);
                }
            },
        });
    }
