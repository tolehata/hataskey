...
>
	...
	<div class="_spacer" style="--MI_SPACER-min: 20px; --MI_SPACER-max: 28px;">
		<div :class="$style.gaps">
			...
			<MkInput v-model="title" :placeholder="copy.titlePlaceholder">
				<template #label>{{ copy.content }} <span :class="$style.req">{{ copy.required }}</span></template>
			</MkInput>

			<MkTextarea v-model="description" :placeholder="copy.descriptionPlaceholder">
			...
			<div :class="$style.navRow">
				<MkButton rounded @click="dialog?.close()">{{ copy.cancel }}</MkButton>
				<MkButton rounded primary gradate :disabled="!title.trim() || submitting" @click="submit"><i class="ti ti-send"></i> {{ copy.publish }}</MkButton>
			</div>
		</div>
	</div>
</MkWindow>
