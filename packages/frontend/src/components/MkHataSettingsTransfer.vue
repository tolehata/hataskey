...
<template>
<MkModalWindow ref="dialog" :width="560" :height="680" :withOkButton="false" :withCloseButton="true" :panelClass="$style.modalPanel" @close="dialog?.close()" @closed="emit('closed')">
	<template #header><i class="ti ti-arrows-exchange"></i> 旗鯖独自設定の入出力</template>
	<div :class="$style.body">
		...
		<template v-if="mode === 'export'">
			...
			<button type="button" :class="$style.primaryButton" :disabled="busy || selectedExport.length === 0" @click="doExport"><i class="ti ti-download"></i> 選んだ設定を書き出す</button>
		...
		<template v-else>
			...
			<button v-if="importFile" type="button" :class="$style.primaryButton" :disabled="busy || selectedImport.length === 0" @click="doImport"><i class="ti ti-device-floppy"></i> 選んだ設定を読み込む</button>
		</template>
	</div>
</MkModalWindow>
</template>
...
.modalPanel { min-height:0; border:1px solid color-mix(in srgb,var(--MI_THEME-divider) 72%,var(--MI_THEME-fg) 28%); background:var(--MI_THEME-bg); background-clip:padding-box; isolation:isolate; box-shadow:0 18px 56px color-mix(in srgb,var(--MI_THEME-shadow) 72%,transparent); }
.body { padding:20px; display:flex; flex-direction:column; gap:16px; color:var(--MI_THEME-fg); }
...
.primaryButton, .fileButton { width:100%; min-height:44px; padding:10px 16px; border:0; border-radius:12px; cursor:pointer; font:inherit; font-weight:800; }
...
.fileButton { color:var(--MI_THEME-fg); background:var(--MI_THEME-panel); border:1px dashed var(--MI_THEME-accent); }
.primaryButton:disabled, .fileButton:disabled { opacity:.45; cursor:not-allowed; }
...
.dedicatedGrid { display:grid; grid-template-columns:repeat(2,minmax(0,1fr)); gap:9px; }
...
@container (max-width: 480px) { .body { padding:14px; } .categoryItem { padding:10px; } .dedicatedGrid { grid-template-columns:1fr; } }
</style>
