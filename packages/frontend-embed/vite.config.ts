...
const url = process.env.NODE_ENV === 'development' ? loadYaml(await fsp.readFile('../../.config/default.yml', 'utf-8')).url : null;
	...
	return {
		...
		build: {
			...
			rollupOptions: {
				...
				output: {
					// 旗鯖fork(G12): vite 8(rolldown) で manualChunks 廃止 → codeSplitting.groups（上流 2026.7.0 と同形）
					codeSplitting: {
						groups: [{
							name: 'vue',
